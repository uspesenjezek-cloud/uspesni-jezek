...
(function () {
  ...
  function zajemiRealtimeFokusActionSheeta() {
    var aktivni = document.activeElement;
    if (!state.actionSheetOpen || !aktivni || !elActionSheet.contains(aktivni)) return null;
    var atributi = Array.prototype.slice.call(aktivni.attributes || []).filter(function (atribut) {
      return atribut.name === "id" || atribut.name === "name" || atribut.name === "type" || atribut.name.indexOf("data-") === 0;
    }).map(function (atribut) {
      return { name: atribut.name, value: atribut.value };
    });
    var drsnik = aktivni.closest && aktivni.closest(".izvedba-action-sheet__scroll");
    return {
      actionSheetMode: state.actionSheetMode,
      currentStepId: state.currentStepId,
      tagName: aktivni.tagName,
      atributi: atributi,
      selectionStart: typeof aktivni.selectionStart === "number" ? aktivni.selectionStart : null,
      selectionEnd: typeof aktivni.selectionEnd === "number" ? aktivni.selectionEnd : null,
      scrollTop: drsnik ? drsnik.scrollTop : null,
    };
  }

  function obnoviRealtimeFokusActionSheeta(posnetek) {
    if (!posnetek || !state.actionSheetOpen || posnetek.actionSheetMode !== state.actionSheetMode || posnetek.currentStepId !== state.currentStepId) return;
    if (document.activeElement && document.activeElement !== document.body && !elActionSheet.contains(document.activeElement)) return;
    var kandidati = Array.prototype.slice.call(elActionSheet.querySelectorAll(String(posnetek.tagName || "").toLowerCase())).filter(function (kontrolnik) {
      return posnetek.atributi.every(function (atribut) { return kontrolnik.getAttribute(atribut.name) === atribut.value; });
    });
    if (kandidati.length !== 1) return;
    var kontrolnik = kandidati[0];
    if (kontrolnik.disabled || kontrolnik.hidden || kontrolnik.closest("[hidden]")) return;
    kontrolnik.focus({ preventScroll: true });
    if (posnetek.selectionStart != null && posnetek.selectionEnd != null && typeof kontrolnik.setSelectionRange === "function") {
      var dolzina = String(kontrolnik.value || "").length;
      kontrolnik.setSelectionRange(Math.min(posnetek.selectionStart, dolzina), Math.min(posnetek.selectionEnd, dolzina));
    }
    var drsnik = kontrolnik.closest && kontrolnik.closest(".izvedba-action-sheet__scroll");
    if (drsnik && posnetek.scrollTop != null) drsnik.scrollTop = posnetek.scrollTop;
  }

  function naloziNajnovejsiPosnetek(ohraniRealtimeFokus) {
    var mojaRealtimeGeneracija = ++state.realtimeGeneracija;
    var zadevaIdObZahtevi = state.zadevaId;
    return Api.nalozi({ zadevaId: zadevaIdObZahtevi }).then(function (odgovor) {
      if (!odgovor || odgovor.ok !== true) return;
      if (mojaRealtimeGeneracija !== state.realtimeGeneracija || zadevaIdObZahtevi !== state.zadevaId) return;
      var novaVerzija = Number((odgovor.plan && odgovor.plan.version) || odgovor.version || 0);
      var trenutnaVerzija = Number(state.serverVersion);
      if (Number.isFinite(novaVerzija) && Number.isFinite(trenutnaVerzija) && novaVerzija < trenutnaVerzija) return;
      var realtimeFokus = ohraniRealtimeFokus ? zajemiRealtimeFokusActionSheeta() : null;
      uporabiOdgovor(odgovor);
      render(realtimeFokus ? { realtimeFokus: realtimeFokus } : undefined);
    }).catch(function () { /* tiho - naslednji realtime dogodek bo poskusil znova */ });
  }

  function osveziIzvedboIzRealtime() {
    return naloziNajnovejsiPosnetek(true);
  }

  function naNoviDogodek() {
    window.clearTimeout(state.debounceTimer);
    state.debounceTimer = window.setTimeout(function () {
      osveziIzvedboIzRealtime();
    }, 150);
  }
  ...
  function obravnavajNapakoUkrepa(odgovor) {
    ...
    if (koda === "VERSION_CONFLICT") {
      ...
      state.pendingActionId = null;
      state.pendingActionType = null;
      naloziNajnovejsiPosnetek(false);
      return;
    }
  ...
  function render(moznosti) {
    var realtimeFokus = moznosti && moznosti.realtimeFokus;
    ...
    izrisiSticky();
    izrisiActionSheet();
    obnoviRealtimeFokusActionSheeta(realtimeFokus);
  ...
  function izrisiPoravnavaRazlog(tip) {
    ...
    var moznosti = razlogi.filter(function (razlog) { return razlog.vrednost; }).map(function (razlog) {
      ...
      return '<button type="button" class="izvedba-poravnava__razlog-moznost' + (jeIzbran ? ' is-selected' : '') + '" role="option" aria-selected="' + String(jeIzbran) + '" data-settlement-reason-option="' + K.esc(razlog.vrednost) + '" data-settlement-type="' + K.esc(tip) + '">' +
        '<span>' + K.esc(razlog.oznaka) + '</span><span class="izvedba-poravnava__razlog-kljukica" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 6 9 17l-5-5"/></svg></span></button>';
    }).join("");
    ...
    var vrstice = !seznam.length
      ...
      var odstraniGumb = korak.jeIzracunaniPreostanek
        ...
        : jeRazveljivUkrep
          ...
          : '<span class="izvedba-poravnava-korak__izveden" aria-label="Korak je že izveden" title="Že izvedeno"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 6 9 17l-5-5"/></svg></span>';
