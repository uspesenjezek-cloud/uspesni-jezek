(function () {
  ...
  var ciljRetrySekunde = -1;
  var ciljRetryGate = window.UJAtenaRequest.createRetryGate({
    onTick: function (remainingMs) {
      var sekunde = Math.ceil(remainingMs / 1000);
      if (sekunde === ciljRetrySekunde) return;
      ciljRetrySekunde = sekunde;
  ...
  function ciljNapakaZRetry() {
    var sporocilo = String(ciljAiNapaka || "");
    if (ciljRetryGate.isBlocked()) sporocilo += " Ponovno lahko poskusite čez " + Math.ceil(ciljRetryGate.remainingMs() / 1000) + " s.";
    return sporocilo;
  }
  ...
  function naravniVnosHtml() {
    ...
    var retryBlokiran = ciljRetryGate.isBlocked();
    ...
    var status = ciljAiNapaka
      ? '<p class="zgodovina-ai__status is-error" aria-live="polite">' + K.esc(ciljNapakaZRetry()) + '</p>'
      : '';
    ...
    return '<section class="zgodovina-ai" aria-label="Povejte ali napišite cilj">' +
      ...
      '<button type="button" class="zgodovina-ai__razumi" data-cilj-pripravi' + (pripravlja ? ' aria-busy="true"' : '') + (pripravlja || recording || retryBlokiran || !naravniOpis.trim() ? ' disabled' : '') + '>' + (pripravlja ? '<span class="izvedba-sticky__loader" aria-hidden="true"></span><span data-ai-analyze-status>' + K.esc(CILJ_ANALIZA_STATUS_BESEDILA[0]) + '</span>' : 'Pripravi cilje') + '</button>' +
      '</div>' + status + '</section>';
  }
  ...
  function ciljAtenaPojasniloHtml() {
    ...
    return '<div class="zgodovina-ai-pogovor zgodovina-ai-pogovor--pojasnilo">' + ciljAtenaVirOpisHtml() + ciljAtenaNapredekHtml(false) + ciljAtenaStanjeDolgaHtml() +
      ...
      '<button type="button" class="zgodovina-ai__potrdi" data-cilj-clarification-submit' + (!ciljClarificationAnswer.trim() || busy || retryBlokiran ? ' disabled' : '') + '>' + (busy ? '<span class="izvedba-sticky__loader" aria-hidden="true"></span> Preverjam …' : 'Odgovori') + '</button></div>' +
      status + '</div></div></div>';
  }
  ...
  function izracunajEnakObrok(ciljniZnesek, steviloObrokov) {
    var znesek = Number(ciljniZnesek);
    var stevilo = Number(steviloObrokov);
    if (!Number.isFinite(znesek) || znesek <= 0 || !Number.isInteger(stevilo) || stevilo < 2) return "";
    var centi = Math.round(znesek * 100);
    if (Math.abs(znesek * 100 - centi) > 0.000001 || centi % stevilo !== 0) return "";
    return (centi / stevilo / 100).toFixed(2).replace(/\.00$/, "").replace(/(\.\d)0$/, "$1");
  }

  function osveziSamodejniZnesekObroka(spremenjenoPolje) {
    if (ciljOsnutekId !== "installment_plan") return;
    if (spremenjenoPolje === "installmentAmount") {
      ciljSamodejniZnesekObroka = "";
      return;
    }
    if (!["targetAmount", "installmentCount"].includes(spremenjenoPolje)) return;
    var trenutni = String(ciljOsnutekPodatki.installmentAmount || "");
    if (trenutni && trenutni !== ciljSamodejniZnesekObroka) return;
    var izracunani = izracunajEnakObrok(ciljOsnutekPodatki.targetAmount, ciljOsnutekPodatki.installmentCount);
    ciljOsnutekPodatki.installmentAmount = izracunani;
    ciljSamodejniZnesekObroka = izracunani;
    var polje = root.querySelector('[data-cilj-polje="installmentAmount"]');
    if (polje && polje.value !== izracunani) {
      polje.value = izracunani;
      if (typeof window.UJIzvedbaFitText === "function") window.UJIzvedbaFitText(polje);
    }
  }
  ...
  function ciljDatumPrikaz(value) {
    var match = String(value || "").match(/^(\d{4})-(\d{2})-(\d{2})$/);
    var meseci = ["jan.", "feb.", "mar.", "apr.", "maj", "jun.", "jul.", "avg.", "sep.", "okt.", "nov.", "dec."];
    return match ? Number(match[3]) + " " + meseci[Number(match[2]) - 1] + " " + match[1] : "dan ? · mesec ? · leto ?";
  }

  function ciljDatumPolje(ime, oznaka, obvezno) {
    var value = ciljVrednost(ime, "");
    return '<label class="zgodovina-dogodek__polje is-date"><span class="zgodovina-ai-vprasanje__oznaka">' + K.esc(oznaka) + (obvezno ? '' : ' <small>neobvezno</small>') + '</span><span class="cilj-datum-prikaz' + (!value ? ' is-empty' : '') + '"><span aria-hidden="true">' + K.esc(ciljDatumPrikaz(value)) + '</span><input type="date" data-cilj-polje="' + K.esc(ime) + '" value="' + K.esc(value) + '"></span></label>';
  }
  ...
  function ciljRokSHitroIzbiroPolje(ime, oznaka) {
    ...
    return '<div class="zgodovina-dogodek__polje cilj-rok-hitri" role="group" aria-labelledby="cilj-rok-hitri-oznaka">' +
      '<label class="cilj-rok-hitri__polje"><span class="zgodovina-ai-vprasanje__oznaka" id="cilj-rok-hitri-oznaka">' + K.esc(oznaka) + '</span><input type="text" maxlength="40" data-cilj-polje="' + K.esc(ime) + '" data-izvedba-fit data-fit-min="9" value="' + K.esc(vrednost) + '" placeholder="dan ? · mesec ? · leto ?"></label>' +
  ...
  function uporabiLuninCilj(priporocilo, ohraniNaravniNacin, indeks) {
    ...
    if (kartica.id === "legal_recovery") {
      ...
      ciljSamodejniZnesekObroka = kartica.id === "installment_plan" && Array.isArray(priporocilo.derivedFields) && priporocilo.derivedFields.includes("installmentAmount")
        ? String(ciljOsnutekPodatki.installmentAmount || "")
        : "";
  ...
  async function pripraviCiljneMoznosti(pojasnilo) {
    ...
    if (!opis || ciljAiStatus === "analyzing" || ciljRetryGate.isBlocked()) return;
    ...
    try {
      ...
      if (podatki.requestId !== ciljAiRequestId || podatki.engineVersion !== "atena-v7" || podatki.contractVersion !== "goal-fact-v18") throw new Error("Atena je bila posodobljena. Osvežite stran in poskusite znova.");
      ...
      if (window.UJAtenaRequest.isRetryable(error)) ciljRetryGate.block(error.retryAfterMs);
      debug.izrisiActionSheet();
    } finally {
  ...
  root.addEventListener("click", function (dogodek) {
    ...
    if (dogodek.target.closest("[data-cilj-clarification-submit]")) {
      ...
      if (!pojasnilo || ciljAiStatus === "analyzing" || ciljRetryGate.isBlocked()) return;
  ...
  root.addEventListener("input", function (dogodek) {
    var datumOvoj = dogodek.target.closest(".cilj-datum-prikaz");
    if (datumOvoj) {
      datumOvoj.classList.toggle("is-empty", !dogodek.target.value);
      datumOvoj.querySelector("span").textContent = ciljDatumPrikaz(dogodek.target.value);
    }
    if (dogodek.target.matches("[data-cilj-clarification-answer]")) {
      ...
      if (odgovori) odgovori.disabled = !ciljClarificationAnswer.trim() || ciljRetryGate.isBlocked();
    }
    if (dogodek.target.matches("[data-cilj-opis]")) {
      ...
      if (pripravi) pripravi.disabled = !naravniOpis.trim() || ciljRetryGate.isBlocked();
    }
    ...
    if (dogodek.target.matches("[data-cilj-polje]")) {
      var ciljPoljeIme = dogodek.target.getAttribute("data-cilj-polje");
      ciljOsnutekPodatki[ciljPoljeIme] = dogodek.target.value;
      osveziSamodejniZnesekObroka(ciljPoljeIme);
      shraniAktivniLuninCilj();
      ciljNapaka = "";
      shraniCilj(false);
  ...
  root.addEventListener("change", function (dogodek) {
    ...
    if (dogodek.target.matches("[data-cilj-polje]")) {
      var ciljSpremenjenoPolje = dogodek.target.getAttribute("data-cilj-polje");
      ciljOsnutekPodatki[ciljSpremenjenoPolje] = dogodek.target.value;
      osveziSamodejniZnesekObroka(ciljSpremenjenoPolje);
      shraniAktivniLuninCilj();
      ciljNapaka = "";
      shraniCilj(false);
    }
