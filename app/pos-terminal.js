(function (global) {
  ...
  var BONITETA_OFFER_TRANSFER_KEY = "uj_boniteta_offer_transfer_v1";
  var BONITETA_OFFER_TRANSFER_MAX_AGE_MS = 24 * 60 * 60 * 1000;
  var BONITETA_OFFER_TRANSFER_FUTURE_SKEW_MS = 5 * 60 * 1000;
  var DATE_LOCALE = "de-DE";
  var CURRENCY = "EUR";
  var MAX_BANK_IMPORT_BYTES = 5 * 1024 * 1024;
  var SUPPORTED_LEGAL_FORMS = ["Einzelunternehmen", "e.K.", "GbR", "eGbR", "OHG", "KG", "GmbH & Co. KG", "UG (haftungsbeschränkt)", "GmbH", "AG", "eG"];
  var REGISTERED_LEGAL_FORMS = ["e.K.", "eGbR", "OHG", "KG", "GmbH & Co. KG", "UG (haftungsbeschränkt)", "GmbH", "AG", "eG"];
  ...
  function parseQuantityMilli(value) {
    // Enaka pravila kot parseMoneyToCents: nemska tisocica je pika, decimalka vejica.
    // Brez tega je "1.000" v polju cene 1000, v polju kolicine pa 1.
    var normalized = String(value == null ? "" : value)
      .trim()
      .replace(/\s/g, "")
      .replace(/\.(?=\d{3}(?:\D|$))/g, "")
  ...
  function formatDecimalMilli(milli, useGrouping) {
    return new Intl.NumberFormat(DATE_LOCALE, { maximumFractionDigits: 3, useGrouping: useGrouping !== false }).format(milli / 1000);
  }
  ...
  var POS_API_TIMEOUT_MS = 30000;
  var POS_DOCUMENT_TIMEOUT_MS = 65000;
  var operationRequestIds = {};

  function posRequestTimeoutError() {
    var error = new Error("Zahteva je potekla. Preverite povezavo in poskusite znova.");
    error.code = "POS_REQUEST_TIMEOUT";
    error.retryable = true;
    return error;
  }

  function withOperationTimeout(operation, timeoutMs) {
    var duration = Math.min(Math.max(integer(timeoutMs, POS_API_TIMEOUT_MS), 10), 120000);
    return new Promise(function (resolve, reject) {
      var settled = false;
      var timer = global.setTimeout(function () {
        if (settled) return;
        settled = true;
        reject(posRequestTimeoutError());
      }, duration);
  ...
  async function posFetch(url, options, timeoutMs, fetchImplementation) {
    var request = Object.assign({}, options || {});
    var fetcher = fetchImplementation || global.fetch;
    if (typeof fetcher !== "function") throw new Error("Omrežna povezava ni na voljo.");
    var duration = Math.min(Math.max(integer(timeoutMs, POS_API_TIMEOUT_MS), 10), 120000);
    if (typeof global.AbortController !== "function") return fetcher(url, request);
    ...
    var timer = global.setTimeout(function () { timedOut = true; controller.abort(); }, duration);
    try {
      return await fetcher(url, request);
    } catch (error) {
      if (timedOut) throw posRequestTimeoutError();
      throw error;
    } finally {
      global.clearTimeout(timer);
  ...
  function missingSingleRowError(error) {
    return Boolean(error && String(error.code || "").toUpperCase() === "PGRST116");
  }
  ...
  function defaultProfile(now) {
    ...
    return {
      ...
      smallBusinessEligibilityConfirmed: false,
      einvoiceReceivingConfirmed: false,
      accountHolder: "",
      iban: "",
  ...
  function normalizeBonitetaItem(source) {
    source = source && typeof source === "object" ? source : {};
    var company = source.company && typeof source.company === "object" ? source.company : {};
    var verification = source.verification && typeof source.verification === "object" ? source.verification : {};
    var status = String(company.identityStatus || verification.identityStatus || "");
    if (["verified_register", "confirmed_impressum"].indexOf(status) === -1) return null;
    if (!String(company.name || "").trim() || !String(company.street || "").trim() || !/^\d{5}$/.test(String(company.postalCode || "").trim()) || !String(company.city || "").trim()) return null;
    return {
      sourceProfileId: String(source.sourceProfileId || ""),
      company: {
        name: String(company.name).trim(),
        street: String(company.street).trim(),
        postalCode: String(company.postalCode).trim(),
        city: String(company.city).trim(),
        entityType: String(company.entityType || "company"),
        registerNumber: String(company.registerNumber || ""),
        registerCourt: String(company.registerCourt || ""),
        companyId: String(company.companyId || ""),
        sourceUrl: String(company.sourceUrl || ""),
        identityStatus: status
      },
      verification: {
        checkedAt: String(verification.checkedAt || ""),
        sourceUrl: String(verification.sourceUrl || company.sourceUrl || ""),
        identityStatus: status,
        registerNumber: String(verification.registerNumber || company.registerNumber || ""),
        registerCourt: String(verification.registerCourt || company.registerCourt || "")
      },
      resultSummary: {
        level: String(source.resultSummary && source.resultSummary.level || ""),
        title: String(source.resultSummary && source.resultSummary.title || "Preverba zaključena"),
        insolvencyStatus: String(source.resultSummary && source.resultSummary.insolvencyStatus || "unverifiable")
      },
      recommendations: (Array.isArray(source.recommendations) ? source.recommendations : []).filter(function (entry) {
        return entry && entry.key === "payment_security_650f";
      }).slice(0, 1).map(function (entry) {
        return { key: "payment_security_650f", selected: Boolean(entry.selected), status: "eligibility_review_required" };
      })
    };
  }

  function normalizeBonitetaContext(value, requireFresh) {
    var source = value && typeof value === "object" ? value : {};
    var transferredAt = String(source.transferredAt || "");
    var expiresAt = String(source.expiresAt || "");
    var version = Number(source.version);
    if (version !== 1 && version !== 2) return null;
    if (requireFresh) {
      var transferredAtMs = Date.parse(transferredAt);
      var expiresAtMs = Date.parse(expiresAt);
      var nowMs = Date.now();
      if (!Number.isFinite(transferredAtMs) || !Number.isFinite(expiresAtMs)
        || transferredAtMs > nowMs + BONITETA_OFFER_TRANSFER_FUTURE_SKEW_MS
        || expiresAtMs <= nowMs
        || expiresAtMs <= transferredAtMs
        || expiresAtMs - transferredAtMs > BONITETA_OFFER_TRANSFER_MAX_AGE_MS + BONITETA_OFFER_TRANSFER_FUTURE_SKEW_MS) return null;
    }
    var rawItems = version === 2 && Array.isArray(source.companies) ? source.companies.slice(0, 2) : [source];
    var companies = rawItems.map(normalizeBonitetaItem);
    if (!companies.length || companies.some(function (item) { return !item; })) return null;
  ...
  function profileToDatabase(profile, userId) {
    return {
      ...
      small_business_eligibility_confirmed: profile.taxStatus === "small_business" && Boolean(profile.smallBusinessEligibilityConfirmed),
      einvoice_receiving_confirmed: Boolean(profile.einvoiceReceivingConfirmed),
  ...
  function profileFromDatabase(row) {
    ...
    return Object.assign(defaultProfile(), {
      ...
      smallBusinessEligibilityConfirmed: Boolean(row.small_business_eligibility_confirmed),
      einvoiceReceivingConfirmed: Boolean(row.einvoice_receiving_confirmed),
  ...
  function draftToDatabasePayload(draft) {
    ...
    return {
      ...
      boniteta_context: normalizeBonitetaContext(draft.bonitetaContext)
    };
  }
  ...
  function draftFromDatabasePayload(payload, issued) {
    ...
    return Object.assign(draft, {
      ...
      bonitetaContext: normalizeBonitetaContext(source.boniteta_context)
    });
  }
  ...
  function createWithdrawalTaxCredit(order) {
    ...
    openDialog("Izdati delni davčni dobropis?", "POS bo iz zaklenjenih postavk računov sam izračunal neto in DDV za zmanjšanje " + formatMoney(correction.reductionCents) + ". Priznani Wertersatz " + formatMoney(order.withdrawalValueCompensationCents) + " ostane obdavčljiv. Dobropis bo nespremenljiv in ne bo sprožil vračila denarja.", {
      ...
      onConfirm: async function () {
        try {
          var result = await withOperationTimeout(backend.client.rpc("pos_create_withdrawal_tax_credit_notes", {
  ...
  function workOrderActions(orderOrStatus, now) {
    var order = orderOrStatus && typeof orderOrStatus === "object" ? orderOrStatus : null;
    var status = order ? order.status : orderOrStatus;
    var source = order && (order.lockedPayload || order.payload) || {};
    // Uro podamo naprej, sicer je odstopni rok merljiv samo proti pravemu datumu
    // in ga ni mogoce determinsticno preveriti.
    var consumerWithdrawal = order && consumerWithdrawalAvailable(order, now);
  ...
  function profileValidationError(profile) {
    ...
    var supportedForms = SUPPORTED_LEGAL_FORMS;
    var registeredForms = REGISTERED_LEGAL_FORMS;
    ...
    if (profile.legalConfirmed && profile.taxStatus === "small_business" && !profile.smallBusinessEligibilityConfirmed) return "Za Kleinunternehmer potrdite veljavne pogoje po § 19 UStG.";
    if (profile.legalConfirmed && (!present(profile.businessEmail) || !profile.einvoiceReceivingConfirmed)) return "Pred produkcijo potrdite dosegljiv poslovni e-poštni predal za prejem strukturiranih e-računov.";
    return "";
  }
  ...
  function profileChangeRequiresConfirmation(fieldName) {
    return [
      ...
      "previousYearTurnoverBand", "smallBusinessEligibilityConfirmed", "einvoiceReceivingConfirmed", "accountHolder", "iban"
  ...
  function profileReadiness(profile) {
    ...
    var checks = [
      ...
      { key: "small-business", label: "Pogoji za Kleinunternehmer", done: profile.taxStatus !== "small_business" || Boolean(profile.smallBusinessEligibilityConfirmed) },
      { key: "einvoice-receiving", label: "Prejem strukturiranih e-računov", done: present(profile.businessEmail) && Boolean(profile.einvoiceReceivingConfirmed) },
  ...
  function publicDeliverySandboxCopy(channel) {
    if (channel === "peppol") return {
      title: "Peppol mock sandbox",
      copy: "Leitweg-ID ostane v arhiviranem XML. Peppol prejemnik in zunanja dostopna točka se samo simulirata; nič se ne prenese.",
      help: "Leitweg-ID ostane v XML; Peppol participant ID in zunanja dostopna točka v mocku nista uporabljena.",
      success: "Peppol mock sandbox je končan. Nič ni bilo preneseno."
    };
    return {
  ...
  var Core = {
    ...
    formatDecimalMilli: formatDecimalMilli,
    ...
    publicDeliverySandboxCopy: publicDeliverySandboxCopy,
    ...
    posFetch: posFetch,
    withOperationTimeout: withOperationTimeout,
    operationRequestId: operationRequestId,
    clearOperationRequestId: clearOperationRequestId,
    operationScopeHash: operationScopeHash,
    missingSingleRowError: missingSingleRowError,
    POS_API_TIMEOUT_MS: POS_API_TIMEOUT_MS,
    POS_DOCUMENT_TIMEOUT_MS: POS_DOCUMENT_TIMEOUT_MS,
  ...
  var deliveryAdjustmentId = null;
  var deliveryAdjustmentEntry = null;
  ...
  var openapiCapability = { provider: "openapi", configured: false, sendEnabled: false, sandboxEnabled: false, liveEnabled: false, financialAdjustmentsEnabled: false, mode: "sandbox" };
  var fiskalyCapability = { configured: false, connected: false, integrationReady: false, environment: "test", tssCount: 0, tssState: "", clientState: "", cashModuleEnabled: false, lastError: false };
  var invoiceIssuing = false;
  ...
  var procedureVersions = [];
  var datevCloudCapability = { loaded: false, loading: false, working: false, configured: false, connected: false, environment: "mock", clientName: "", consultantNumber: null, clientNumber: null, latestTransfer: null, lastError: "" };
  var toastTimer = 0;
  ...
  function loadState() {
    var initial = { profile: defaultProfile(), invoices: [], workOrders: [], bankTransactions: [], cashCheckouts: [], cashMovements: [], draft: null, sequence: 0, storageOwnerUserId: "" };
    try {
      ...
      return {
        ...
        cashCheckouts: Array.isArray(saved.cashCheckouts) ? saved.cashCheckouts : [],
        cashMovements: Array.isArray(saved.cashMovements) ? saved.cashMovements : [],
  ...
  async function getBackendUser() {
    ...
    if (nextUserId && state.storageOwnerUserId && state.storageOwnerUserId !== nextUserId) {
      state = { profile: defaultProfile(), invoices: [], workOrders: [], bankTransactions: [], cashCheckouts: [], cashMovements: [], draft: null, sequence: 0, storageOwnerUserId: "" };
  ...
  async function saveDraftToServer(options) {
    ...
    syncDraftFromForm();
    var existingDraftId = state.draft.serverId || "";
    var row = { user_id: backend.userId, payload: draftToDatabasePayload(state.draft) };
    var request;
    if (existingDraftId) {
      request = backend.client.from("pos_invoice_drafts").update(row).eq("id", existingDraftId).select("id").single();
    } else {
      request = backend.client.from("pos_invoice_drafts").insert(row).select("id").single();
    }
    var result = await request;
    if (result.error) {
      if (existingDraftId && options && options.allowIssuedRetry && missingSingleRowError(result.error)) return existingDraftId;
      throw result.error;
    }
  ...
  function cashReceiptForInvoice(invoice) {
    if (!invoice || !invoice.isTest || invoice.status === "cancelled" || invoice.status === "credited" || invoice.hasCreditNote) return null;
    var outstanding = invoiceOutstandingCents(invoice);
    var draft = invoice.draft || {};
    if (outstanding <= 0 || draft.workflowContext && Array.isArray(draft.workflowContext.finalDeductions) && draft.workflowContext.finalDeductions.length) return null;
    var items = (draft.items || []).map(function (item) {
      var calculated = calculateItem(item, draft.priceMode, draft.taxMode);
      return {
        description: String(item.description || "").trim(),
        quantityMilli: calculated.quantityMilli,
        unitGrossCents: calculated.quantityMilli > 0 ? roundDivide(calculated.grossCents * 1000, calculated.quantityMilli) : 0,
        grossCents: calculated.grossCents,
        vatRate: String(Math.round(calculated.rateBps / 100))
      };
    }).filter(function (item) { return item.description && item.quantityMilli > 0 && item.grossCents >= 0; });
    if (!items.length || items.reduce(function (sum, item) { return sum + item.grossCents; }, 0) !== outstanding) return null;
    var vatMap = {};
    items.forEach(function (item) {
      var rate = integer(item.vatRate, 0);
      var net = rate ? roundDivide(item.grossCents * 100, 100 + rate) : item.grossCents;
      if (!vatMap[item.vatRate]) vatMap[item.vatRate] = { vatRate: item.vatRate, netCents: 0, taxCents: 0, grossCents: 0 };
      vatMap[item.vatRate].netCents += net;
      vatMap[item.vatRate].taxCents += item.grossCents - net;
      vatMap[item.vatRate].grossCents += item.grossCents;
    });
    return { schemaVersion: 1, paymentType: "CASH", currency: "EUR", grossCents: outstanding, items: items, totalsByVat: Object.keys(vatMap).sort().map(function (rate) { return vatMap[rate]; }) };
  }

  function applyLocalCashCheckouts(invoices, checkouts) {
    var result = Array.isArray(invoices) ? invoices : [];
    (Array.isArray(checkouts) ? checkouts : []).filter(function (checkout) { return checkout && checkout.state === "completed" && checkout.invoiceId; }).forEach(function (checkout) {
      var invoice = result.filter(function (entry) { return entry.id === checkout.invoiceId; })[0];
      if (!invoice) return;
      var payments = invoice.payments || [];
      var existingPayment = payments.filter(function (payment) { return payment.id === checkout.paymentId; })[0];
      if (!existingPayment) {
        payments = payments.concat([{
          id: checkout.paymentId || "cash-" + checkout.id,
          invoiceId: invoice.id,
          amountCents: integer(checkout.receipt && checkout.receipt.grossCents, 0),
          currency: "EUR",
          method: "cash",
          provider: "fiskaly",
          providerReference: "Lokaler TSE-Mock · " + String(checkout.signature && checkout.signature.signatureCounter || "–"),
          paidAt: checkout.completedAt || checkout.signature && checkout.signature.finishedAt || null,
          status: (checkout.refund && checkout.refund.state === "completed") || checkout.refundedAt ? "refunded" : "succeeded",
          refundedCents: (checkout.refund && checkout.refund.state === "completed") || checkout.refundedAt ? integer(checkout.receipt && checkout.receipt.grossCents, 0) : 0,
          metadata: { training: true, cashCheckoutId: checkout.id }
        }]);
      } else if ((checkout.refund && checkout.refund.state === "completed") || checkout.refundedAt) {
        existingPayment.status = "refunded";
        existingPayment.refundedCents = integer(checkout.receipt && checkout.receipt.grossCents, 0);
      }
      invoice.payments = payments;
      var locked = invoice.status === "cancelled" ? "cancelled" : invoice.status === "credited" ? "credited" : "open";
      var summary = paymentSummary(payments, invoice.adjustedGrossCents == null ? invoice.totals && invoice.totals.grossCents : invoice.adjustedGrossCents, locked);
      invoice.paidCents = summary.paidCents;
  ...
  function serverInvoiceToLocal(row, paymentsByInvoice, documentsByInvoice, adjustmentsByInvoice, deliveriesByInvoice, einvoiceDocumentsByInvoice) {
    ...
    if (!effectivePayload.customer_type && row.customer_type) {
      effectivePayload = Object.assign({}, effectivePayload, { customer_type: row.customer_type });
    }
  ...
  function applyPaymentRefresh(rows) {
    ...
    applyLocalCashCheckouts(state.invoices, state.cashCheckouts);
  }
  ...
  async function loadFullServerState(scopes) {
    ...
    try {
      ...
      var responses = await Promise.all([
        ...
        fetchAllRows(function () { return backend.client.from("pos_payments").select("id,invoice_id,amount_cents,currency,method,provider,provider_reference,paid_at,source_bank_transaction_id,status,refunded_cents,failure_code,checkout_session_id,external_payment_id,expires_at,metadata,created_at").eq("user_id", userId).order("created_at", { ascending: true }).order("id", { ascending: true }); }),
  ...
  function modalFocusables(backdrop) {
    return queryAll("button:not([disabled]), a[href], input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex='-1'])", backdrop)
      .filter(function (element) { return !element.hidden && element.getClientRects().length > 0; });
  }

  function syncModalOpenState() {
    var open = modalStack.some(function (entry) { return entry.backdrop && !entry.backdrop.hidden; });
    document.documentElement.classList.toggle("uj-modal-odprt", open);
    document.body.classList.toggle("uj-modal-odprt", open);
  }

  function focusModalEntry(entry) {
    if (!entry || !entry.backdrop || entry.backdrop.hidden) return;
    var preferred = typeof entry.preferredFocus === "function" ? entry.preferredFocus() : entry.preferredFocus;
    var target = preferred && !preferred.hidden && !preferred.disabled && preferred.getClientRects().length ? preferred : modalFocusables(entry.backdrop)[0];
    if (!target) target = query("[role=dialog]", entry.backdrop);
    if (target && typeof target.focus === "function") target.focus();
  }

  function activateModal(backdrop, close, preferredFocus) {
    var existing = modalStack.findIndex(function (entry) { return entry.backdrop === backdrop; });
    if (existing !== -1) modalStack.splice(existing, 1);
    var entry = { backdrop: backdrop, close: close, preferredFocus: preferredFocus, opener: document.activeElement };
    backdrop.hidden = false;
    modalStack.push(entry);
    syncModalOpenState();
    global.setTimeout(function () {
      if (modalStack[modalStack.length - 1] === entry) focusModalEntry(entry);
    }, 0);
  }

  function deactivateModal(backdrop) {
    var index = -1;
    for (var cursor = modalStack.length - 1; cursor >= 0; cursor -= 1) {
      if (modalStack[cursor].backdrop === backdrop) { index = cursor; break; }
    }
    var entry = index === -1 ? null : modalStack.splice(index, 1)[0];
    backdrop.hidden = true;
    syncModalOpenState();
    global.setTimeout(function () {
      var opener = entry && entry.opener;
      if (opener && opener.isConnected && !opener.hidden && typeof opener.focus === "function") opener.focus();
      else focusModalEntry(modalStack[modalStack.length - 1]);
    }, 0);
  }

  function handleModalKeydown(event) {
    var entry = modalStack[modalStack.length - 1];
    if (!entry || !entry.backdrop || entry.backdrop.hidden) return;
    if (event.key === "Escape") {
    ...
    var focusables = modalFocusables(entry.backdrop);
    ...
    var focusOutside = !entry.backdrop.contains(document.activeElement);
    if ((event.shiftKey && (document.activeElement === first || focusOutside)) || (!event.shiftKey && (document.activeElement === last || focusOutside))) {
      event.preventDefault();
  ...
  function openDialog(title, copy, options) {
    ...
    activateModal(backdrop, function () { closeDialog(false); }, inputOptions ? control : query("[data-dialog-confirm]"));
  ...
  function closeDialog(confirmed) {
    ...
    deactivateModal(query("[data-dialog-backdrop]"));
    var callback = dialogCallback;
    dialogCallback = null;
    dialogValidator = null;
    if (confirmed && callback) {
      try {
        var rezultat = callback(inputValue);
        if (rezultat && typeof rezultat.then === "function") {
          rezultat.catch(function (error) {
            showToast(error && error.message || "Dejanja ni bilo mogoče dokončati.");
          });
        }
      } catch (error) {
        showToast(error && error.message || "Dejanja ni bilo mogoče dokončati.");
  ...
  function syncProfileTaxFields() {
    var form = query("#pos-profile-form");
    if (!form) return;
    var selected = query("[name=taxStatus]:checked", form);
    var confirmation = query("[data-small-business-confirmation]", form);
    var checkbox = query("[name=smallBusinessEligibilityConfirmed]", form);
    var smallBusiness = Boolean(selected && selected.value === "small_business");
    if (confirmation) confirmation.hidden = !smallBusiness;
    if (checkbox) {
      checkbox.disabled = !smallBusiness;
      if (!smallBusiness) checkbox.checked = false;
    }
  }
  ...
  function workOrderActionLabel(action) {
    return {
      ...
      progress: "Abschlagsrechnung", final: "Schlussrechnung", cancel: "Storniraj projekt", withdraw: "Zabeleži odstop",
      ...
      onConfirm: function (acceptedOn) {
        openDialog("Dokaz sprejema", "Potrdite le, če je naročnik ponudbo dejansko sprejel. Dokaz bo vezan na nespremenljivi PDF ponudbe.", {
          ...
          onConfirm: async function (evidence) {
            try {
              var result = await withOperationTimeout(backend.client.rpc("pos_accept_work_order", {
                p_work_order_id: order.id,
    ...
    openDialog("Datum potrošnikove izjave", "Vnesite datum, ko je potrošnik izjavo poslal. Za pravočasnost zadošča pravočasna odpošiljka, zato je ta datum lahko pred datumom prejema.", {
      ...
      onConfirm: function (declaredOn) {
        openDialog("Zabeleži potrošnikov odstop?", "S tem takoj ustavite nadaljnje delo in nove račune. Obstoječi računi, plačila in morebitni Wertersatz se ne spremenijo samodejno in zahtevajo ločen pregled.", {
          ...
          onConfirm: async function (evidence) {
            try {
              var result = await withOperationTimeout(backend.client.rpc("pos_record_consumer_withdrawal", {
                p_work_order_id: order.id,
  ...
  function assessConsumerWithdrawalSettlement(order) {
    ...
    var submit = async function (valueCompensationCents, refundMethod, alternativeEvidence, reason) {
      try {
        var result = await withOperationTimeout(backend.client.rpc("pos_assess_consumer_withdrawal_settlement", {
          p_work_order_id: order.id,
                ...
                openDialog("Referenca vračila", "Vpišite bančno referenco, Stripe refund ID ali drugo enolično oznako izvedene transakcije.", {
                  ...
                  onConfirm: function (reference) {
                    openDialog("Dokaz izvedenega vračila", "Vpišite preverljivo evidenco, na primer bančni izpisek ali Stripe dogodek. Zapis bo nespremenljiv.", {
                      ...
                      onConfirm: async function (evidence) {
                        try {
                          var result = await withOperationTimeout(backend.client.rpc("pos_record_consumer_withdrawal_refund", {
                            p_work_order_id: order.id,
            ...
            openDialog("Dokaz izročitve", "Vpišite preverljivo referenco, ki dokazuje, da je potrošnik potrdilo dejansko prejel.", {
              ...
              onConfirm: function (evidence) {
                var save = function (consentEvidence) {
                  withOperationTimeout(backend.client.rpc("pos_record_contract_confirmation_delivery", {
                    p_work_order_id: order.id,
                    p_channel: channel,
                    p_evidence: String(evidence || "").trim(),
      ...
      onConfirm: function (acceptanceEvidence) {
        var execute = async function (acknowledgements) {
          ...
          try {
            ...
            var result = await withOperationTimeout(request.single());
            if (result.error) throw result.error;
            var offerPdfError = "";
            if (action === "offer") {
  ...
  async function loadArchiveCapability(showFeedback, verify) {
    ...
    try {
      ...
      var response = await posFetch("/api/pos-arhiv", {
  ...
  function renderProcedureVersions() {
    var root = query("[data-procedure-versions]");
    if (!root) return;
    root.hidden = !procedureVersions.length;
    root.innerHTML = procedureVersions.slice(0, 5).map(function (version) {
      return "<button class=\"pos-procedure-version\" type=\"button\" data-procedure-version-id=\"" + escapeHtml(version.id) + "\"><span><strong>Arhivirana različica " + escapeHtml(version.versionNumber) + "</strong><small>" + escapeHtml(formatGermanTimestampDate(version.createdAt)) + " · hramba do " + escapeHtml(formatDate(version.retentionNotBefore)) + "</small></span><svg><use href=\"#i-export\"/></svg></button>";
    }).join("");
  }

  async function loadProcedureVersions() {
    if (!backend.ready) { procedureVersions = []; renderProcedureVersions(); return; }
    try {
      var token = await apiSessionToken();
      var response = await posFetch("/api/pos-verfahrensdokumentation-pdf?mode=list", { headers: { Authorization: "Bearer " + token } });
      var data = await response.json().catch(function () { return {}; });
      if (!response.ok || !data.ok) throw new Error(data.napaka || "Različic Verfahrensdokumentation ni bilo mogoče naložiti.");
      procedureVersions = Array.isArray(data.versions) ? data.versions : [];
    } catch (_) {
      procedureVersions = [];
    }
    renderProcedureVersions();
  }

  async function downloadProcedureDocumentation(versionId) {
    versionId = typeof versionId === "string" ? versionId : "";
    var button = query("[data-procedure-documentation]");
    if (!backend.ready) { showToast("Verfahrensdokumentation zahteva varno prijavo in shranjene podatke podjetja."); return; }
    if (button) button.disabled = true;
    try {
      var token = await apiSessionToken();
      var response = await posFetch("/api/pos-verfahrensdokumentation-pdf" + (versionId ? "?versionId=" + encodeURIComponent(versionId) : ""), { headers: { Authorization: "Bearer " + token } }, POS_DOCUMENT_TIMEOUT_MS);
      if (!response.ok) {
        var errorBody = await response.json().catch(function () { return {}; });
        throw new Error(errorBody.napaka || "Verfahrensdokumentation ni bilo mogoče ustvariti.");
      }
      var version = String(response.headers.get("X-UJ-Document-Version") || "").replace(/[^0-9]/g, "");
      var blob = await response.blob();
      var anchor = document.createElement("a");
      anchor.href = URL.createObjectURL(blob);
      anchor.download = String(state.profile.legalName || "Unternehmen").replace(/[^A-Za-z0-9._-]+/g, "-") + "-Verfahrensdokumentation" + (version ? "-v" + version : "") + ".pdf";
      document.body.appendChild(anchor);
      anchor.click();
      anchor.remove();
      setTimeout(function () { URL.revokeObjectURL(anchor.href); }, 1000);
      showToast(version ? "Verfahrensdokumentation PDF je arhiviran kot različica " + version + "." : "Verfahrensdokumentation PDF je pripravljen.");
      if (!versionId) await loadProcedureVersions();
    } catch (error) {
      showToast(error && error.message || "Verfahrensdokumentation ni bilo mogoče ustvariti.");
    } finally {
  ...
  function renderAdjustmentList(invoice) {
    ...
    function adjustmentRowHtml(entry) {
      ...
      var structured = invoice.draft.customerType === "business" || invoice.draft.customerType === "public";
      ...
      var latestDelivery = (invoice.deliveries || []).filter(function (delivery) { return delivery.adjustmentId === entry.id; }).slice(-1)[0];
      var deliveryCopy = latestDelivery ? deliveryStatusLabel(latestDelivery.status, latestDelivery) : "Pošlji";
      return "<article class=\"pos-adjustment-row " + (financial ? "is-cancellation" : "") + (creditNote ? " is-credit-note" : "") + "\"><span class=\"pos-adjustment-row__icon\"><svg><use href=\"#" + (cancellation ? "i-trash" : creditNote ? "i-receipt" : "i-info") + "\"/></svg></span><div class=\"pos-adjustment-row__copy\"><strong data-fit-text data-fit-max=\"11\">" + escapeHtml(title + " · " + entry.number) + "</strong><small data-fit-text data-fit-max=\"9\">" + escapeHtml(formatDate(berlinDateKey(entry.createdAt)) + " · " + entry.reason) + "</small></div><div class=\"pos-adjustment-row__actions\"><button type=\"button\" data-download-adjustment=\"" + escapeHtml(entry.id) + "\">" + stateCopy + "</button>" + (structured ? "<button type=\"button\" data-download-adjustment-xrechnung=\"" + escapeHtml(entry.id) + "\">" + xmlCopy + "</button><button type=\"button\" data-deliver-adjustment=\"" + escapeHtml(entry.id) + "\">" + escapeHtml(deliveryCopy) + "</button>" : "") + "</div></article>";
    }
    ...
    queryAll("[data-deliver-adjustment]", list).forEach(function (button) {
      button.addEventListener("click", function () {
        var entry = downloadable.filter(function (item) { return item.id === button.getAttribute("data-deliver-adjustment"); })[0];
        if (entry) openAdjustmentDeliverySheet(invoice, entry);
      });
    });
  ...
  function renderDeliveryList(invoice) {
    ...
    queryAll("[data-retry-delivery]", list).forEach(function (button) {
      button.addEventListener("click", async function () {
        ...
        try {
          var retryProvider = button.getAttribute("data-provider");
          if (retryProvider === "resend") await posDeliveryEmailRequest(button.getAttribute("data-retry-delivery"));
          else if (retryProvider === "openapi") await posDeliveryOpenapiRequest(button.getAttribute("data-retry-delivery"));
          else await queueAndRunSandbox(button.getAttribute("data-retry-delivery"));
          await loadServerState("deliveries");
          activeInvoiceId = invoice.id;
          showView("invoice-detail");
          showToast(retryProvider === "resend" ? "E-poštna dostava je ponovno zagnana." : retryProvider === "openapi" ? "Openapi dostava je ponovno zagnana." : "Sandbox preizkus je končan. Nič ni bilo poslano.");
        } catch (error) {
  ...
  function isActiveStripeAttempt(payment) {
    if (!payment || !payment.checkoutSessionId) return false;
    return payment.status === "pending" || payment.status === "failed" ||
      (payment.status === "cancelled" && payment.failureCode !== "checkout_expired");
  }
  ...
  function renderStripePayment(invoice) {
    ...
    var abandonButton = query("[data-stripe-abandon]");
    ...
    if (status === "failed") { title = "Plačilo ni uspelo"; copy = "Stripe Checkout je morda še odprt. Nadaljujte isto sejo ali preverite njeno stanje."; label = "Nadaljuj plačilo – TEST"; }
    if (status === "cancelled" && isActiveStripeAttempt(payment)) { title = "Preklic preverjamo"; copy = "Stripe mora še potrditi, ali je seja res zaprta."; label = "Preveri sejo – TEST"; }
    if (status === "cancelled" && !isActiveStripeAttempt(payment)) { title = "Preklicano"; copy = "Stripe TEST seja je bila varno zaprta brez plačila."; label = "Začni novo plačilo – TEST"; }
    ...
    abandonButton.hidden = !isActiveStripeAttempt(payment);
    abandonButton.disabled = !invoice.serverStored || !backend.ready;
  ...
  function cashCheckoutTransaction(checkout, useRefund) {
    var event = useRefund && checkout && checkout.refund || checkout;
    var signature = event && event.signature || {};
    var receipt = event && event.receipt || checkout && checkout.receipt;
    if (useRefund && receipt) {
      receipt = Object.assign({}, receipt, {
        grossCents: -integer(receipt.grossCents, 0),
        items: (receipt.items || []).map(function (item) { return Object.assign({}, item, { grossCents: -integer(item.grossCents, 0), unitGrossCents: -integer(item.unitGrossCents, 0) }); }),
        totalsByVat: (receipt.totalsByVat || []).map(function (row) { return Object.assign({}, row, { netCents: -integer(row.netCents, 0), taxCents: -integer(row.taxCents, 0), grossCents: -integer(row.grossCents, 0) }); })
      });
    }
    return {
      state: "FINISHED",
      fiscalType: useRefund ? "REFUND" : "SALE",
      transactionNumber: signature.signatureCounter || event && event.id,
      paymentType: "CASH",
      amount: (integer(receipt && receipt.grossCents, 0) / 100).toFixed(2),
      currency: "EUR",
      receipt: receipt,
      signatureCounter: signature.signatureCounter,
      signatureAlgorithm: signature.signatureAlgorithm,
      tssSerialNumber: signature.tssSerialNumber,
      clientSerialNumber: signature.clientSerialNumber,
      qrCodeData: signature.qrCodeData,
      startedAt: signature.startedAt,
      finishedAt: signature.finishedAt || event && event.completedAt
    };
  }

  function renderCashPayment(invoice) {
    ...
    openDialog(reconcileCheckout ? "TSE poskus uskladiti?" : "Gotovino zaključiti s TSE mockom?", invoice.number + " · " + formatMoney(receipt.grossCents) + (reconcileCheckout ? ". Preverjeno bo točno stanje obstoječe TSE transakcije brez novega podpisa." : ". Plačilo bo lokalno zapisano šele po popolnem mock TSE podpisu."), {
      ...
      onConfirm: async function () {
        ...
        try {
          ...
          if (!response.ok || !body || !body.ok || !body.checkout) throw new Error(body && body.napaka || "Lokalni gotovinski checkout ni uspel.");
          var checkout = Object.assign({}, body.checkout, { fiscalInvoiceId: body.checkout.invoiceId, invoiceId: invoice.id, invoiceNumber: invoice.number, receipt: receipt });
  ...
  function showLocalCashReceipt(invoice) {
    var checkout = localCashCheckoutForInvoice(invoice);
    if (!checkout) return;
    renderSignedKassenbon(cashCheckoutTransaction(checkout, Boolean(checkout.refund && checkout.refund.state === "completed")));
    activateModal(query("[data-fiskaly-receipt-backdrop]"), closeFiskalyReceiptSheet, query("[data-fiskaly-receipt-close]"));
  }
    ...
    openDialog(reconcileRefund ? "TSE povračilo uskladiti?" : "Gotovino vrniti?", invoice.number + " · " + formatMoney(amount) + (reconcileRefund ? ". Preverjeno bo točno stanje obstoječe TSE transakcije brez novega podpisa." : ". Povračilo bo zapisano šele po lastnem mock TSE podpisu."), {
      ...
      onConfirm: async function () {
        ...
        try {
          ...
          var body = null;
          try { body = await response.json(); } catch (_error) {}
          if (!response.ok || !body || !body.ok || !body.refund) throw new Error(body && body.napaka || "Lokalno gotovinsko povračilo ni uspelo.");
  ...
  function recordLocalCashMovement(type) {
    var deposit = type === "DEPOSIT";
    openDialog(deposit ? "Gotovinski polog · TEST" : "Gotovinski dvig · TEST", "Dogodek bo ostal samo v lokalnem TRAINING Kassenbuchu.", {
      confirmText: deposit ? "Potrdi polog" : "Potrdi dvig",
      input: { label: "Znesek EUR", value: "0,00", inputMode: "decimal", placeholder: "0,00" },
      validate: function (value) { return parseMoneyToCents(value) > 0 ? "" : "Vnesite pozitiven znesek."; },
      onConfirm: function (value) {
        var movement = { type: type, amountCents: parseMoneyToCents(value), occurredAt: new Date().toISOString(), reference: uid("cash-movement"), reason: deposit ? "Privateinlage TRAINING" : "Privatentnahme TRAINING" };
        global.UJPosDsfinvk.normalizeMovement(movement);
        state.cashMovements = (state.cashMovements || []).concat([movement]);
        persist();
        showToast(deposit ? "Gotovinski polog je zabeležen lokalno." : "Gotovinski dvig je zabeležen lokalno.");
      }
    });
  }
  ...
  function downloadDsfinvk() {
    try {
      if (!global.UJPosDsfinvk) throw new Error("DSFinV-K lokalni modul ni naložen.");
      var result = global.UJPosDsfinvk.buildPackage(state.cashCheckouts, state.profile, { movements: state.cashMovements });
      downloadFile(result.filename, result.content, "application/json;charset=utf-8");
      showToast("DSFinV-K TRAINING paket je pripravljen.");
    } catch (error) { showToast(error && error.message || "DSFinV-K TEST izvoza ni bilo mogoče pripraviti."); }
  }
  ...
  function renderPaymentList(invoice) {
    ...
    list.innerHTML = payments.map(function (payment) {
      ...
      var isCash = payment.method === "cash";
  ...
  function syncDeliveryMode() {
    ...
    var adjustment = deliveryAdjustmentEntry && deliveryAdjustmentEntry.id === deliveryAdjustmentId ? deliveryAdjustmentEntry : null;
    var isAdjustment = Boolean(adjustment);
    ...
    if (isAdjustment) recommendation = {
      channel: type === "public" ? "ozg_re" : "email",
      documentFormat: type === "public" ? "xrechnung" : "xrechnung_pdf",
      title: type === "public" ? "XRechnung prek uradnega kanala" : "Strukturirani popravek po e-pošti",
      copy: "Popravek se pošlje kot KoSIT-potrjen XML in se jasno sklicuje na original.",
      pdfConsentRequired: false, pdfAllowed: false
    };
    ...
    queryAll("option", format).forEach(function (option) {
      if (isAdjustment) option.disabled = type === "public" ? option.value !== "xrechnung" : option.value === "pdf";
      else if (type === "private") option.disabled = option.value !== "pdf";
    ...
    format.disabled = isAdjustment && type === "public" || !isAdjustment && (type === "private" || type === "public");
    ...
    var publicHelp = query("[data-delivery-public-help]");
    if (publicHelp && isPublic) publicHelp.textContent = publicDeliverySandboxCopy(channel.value).help;
    ...
    var einvoiceDocument = isAdjustment ? adjustment.einvoiceDocument : invoice.einvoiceDocument;
    var einvoiceStatus = einvoiceDocument && (einvoiceDocument.validation_status || einvoiceDocument.validationStatus) || "pending";
    ...
    query("[data-delivery-validation-copy]").textContent = !isStructured
      ...
      : einvoiceStatus === "validated" ? (isAdjustment ? "Arhivirani popravek XML je prestal uradno konfiguracijo XRechnung." : "Arhivirani XML je prestal uradno konfiguracijo XRechnung.")
  ...
  function syncDeliveryCapabilityUi() {
    var form = query("#pos-delivery-form");
    var emailChannel = Boolean(form && form.elements.deliveryChannel.value === "email");
    var invoice = findInvoice(deliveryInvoiceId);
    var adjustment = deliveryAdjustmentEntry && deliveryAdjustmentEntry.id === deliveryAdjustmentId ? deliveryAdjustmentEntry : null;
    var openapiEligibleAdjustment = Boolean(adjustment && ["cancellation", "credit_note"].indexOf(adjustment.type) !== -1);
    var structuredInvoice = Boolean(invoice && (!adjustment || openapiEligibleAdjustment) && (invoice.draft.customerType === "business" || invoice.draft.customerType === "public"));
    var financialOpenapiBlocked = Boolean(openapiEligibleAdjustment && structuredInvoice && !openapiCapability.financialAdjustmentsEnabled);
    var useOpenapi = Boolean(structuredInvoice && openapiCapability.sendEnabled);
    var send = useOpenapi || Boolean(emailChannel && deliveryCapability && deliveryCapability.sendEnabled);
    var test = useOpenapi ? Boolean(openapiCapability.sandboxEnabled) : Boolean(emailChannel && deliveryCapability && deliveryCapability.testEnabled);
    var live = useOpenapi ? Boolean(openapiCapability.liveEnabled) : Boolean(emailChannel && deliveryCapability && deliveryCapability.liveEnabled);
    var publicSandbox = !emailChannel && form ? publicDeliverySandboxCopy(form.elements.deliveryChannel.value) : null;
    var note = query("[data-delivery-mode-note]");
    if (!note) return;
    var submit = query("[data-delivery-submit]");
    if (financialOpenapiBlocked) {
      note.classList.remove("is-live");
      note.classList.add("is-test");
      query("[data-delivery-mode-title]").textContent = "Openapi tip 381 čaka produkcijsko aktivacijo";
      query("[data-delivery-mode-copy]").textContent = "Sandbox Storno in Gutschrift sta uspešno dostavljena. Produkcijska oddaja ostaja ločeno zaklenjena.";
      query("[data-delivery-confirm-copy]").textContent = "Prenos ni dovoljen, dokler produkcijska aktivacija ni izrecno potrjena.";
      if (submit) { submit.textContent = "Produkcijska aktivacija še ni potrjena"; submit.disabled = true; }
      return;
    }
    if (submit && !deliverySubmitting) submit.disabled = false;
    note.classList.toggle("is-live", send);
    note.classList.toggle("is-test", test);
    query("[data-delivery-mode-title]").textContent = useOpenapi && test ? "Openapi Invoice sandbox" : useOpenapi && live ? "Openapi Invoice produkcija" : test ? "Varni e-poštni test" : live ? "Pravo e-poštno pošiljanje" : publicSandbox ? publicSandbox.title : "Varen sandbox";
    query("[data-delivery-mode-copy]").textContent = useOpenapi && test
      ? "Testni podatki bodo poslani v Openapi sandbox. Ne nastane pravna dostava prejemniku."
      : useOpenapi && live ? "Openapi bo izdelal ZUGFeRD oziroma XRechnung in izbral Peppol ali e-pošto."
      : test ? "Račun bo dejansko poslan samo na strežniško določen testni naslov. Stranka ga ne bo prejela."
      : live ? "Po potrditvi bo račun z izbranimi prilogami dejansko poslan prejemniku."
      : emailChannel ? "Preverimo celoten potek, vendar dokument ne zapusti sistema in ni dejansko poslan."
        : publicSandbox.copy;
    query("[data-delivery-confirm-copy]").textContent = useOpenapi && test
      ? "S potrditvijo dovolim prenos testnih podatkov v Openapi sandbox."
      : useOpenapi && live ? "S potrditvijo dovolim dejansko oddajo računa ponudniku Openapi."
      : test ? "S potrditvijo dovolim testno pošiljanje samo na dovoljeni testni naslov."
      : live ? "S potrditvijo dovolim dejansko pošiljanje na prikazani e-poštni naslov."
      : publicSandbox ? "Mock preizkus se nespremenljivo zabeleži, brez zunanjega prenosa." : "Sandbox zabeleži preizkus, brez zunanjega pošiljanja.";
    if (submit && !deliverySubmitting) submit.textContent = useOpenapi && test ? "Pošlji v Openapi sandbox" : test ? "Pošlji test" : live ? "Pošlji račun" : publicSandbox ? "Zaženi mock sandbox" : "Zaženi sandbox";
  }
  ...
  async function loadDeliveryCapability() {
    ...
    try {
      var token = await apiSessionToken();
      var response = await posFetch("/api/pos-dostava-email", { method: "GET", headers: { Authorization: "Bearer " + token } });
      var body = null;
      try { body = await response.json(); } catch (_error) {}
    ...
    try {
      var openapiResponse = await posFetch("/api/pos?handler=openapi-invoice", { method: "GET", headers: { Authorization: "Bearer " + token } });
      var openapiBody = null;
      try { openapiBody = await openapiResponse.json(); } catch (_openapiError) {}
      if (!openapiResponse.ok || !openapiBody || !openapiBody.invoice) throw new Error("Stanje Openapi ponudnika ni dosegljivo.");
      openapiCapability = {
        provider: "openapi",
        configured: Boolean(openapiBody.invoice.configured),
        sendEnabled: Boolean(openapiBody.invoice.sendEnabled),
        sandboxEnabled: Boolean(openapiBody.invoice.sandboxEnabled),
        liveEnabled: Boolean(openapiBody.invoice.liveEnabled),
        financialAdjustmentsEnabled: Boolean(openapiBody.invoice.financialAdjustmentsEnabled),
        mode: String(openapiBody.invoice.mode || "sandbox")
      };
    } catch (_openapiError) {
      openapiCapability = { provider: "openapi", configured: false, sendEnabled: false, sandboxEnabled: false, liveEnabled: false, financialAdjustmentsEnabled: false, mode: "sandbox" };
    }
  ...
  function renderFiskalyCapability() {
    ...
    var failed = Boolean(fiskalyCapability.lastError || fiskalyCapability.configured && !fiskalyCapability.connected);
  ...
  async function loadFiskalyCapability(showFeedback) {
    try {
      var token = await apiSessionToken();
      var response = await posFetch("/api/pos-fiskaly", { method: "GET", headers: { Authorization: "Bearer " + token } });
      var body = null;
      try { body = await response.json(); } catch (_error) {}
      ...
      fiskalyCapability = { configured: false, connected: false, integrationReady: false, environment: "test", tssCount: 0, tssState: "", clientState: "", cashModuleEnabled: false, lastError: true };
    }
  ...
  function closeFiskalyReceiptSheet() {
    deactivateModal(query("[data-fiskaly-receipt-backdrop]"));
  }
  ...
  function renderSignedKassenbon(transaction) {
    ...
    var isRefund = transaction.fiscalType === "REFUND";
    ...
    query("[data-kassenbon-kind]").textContent = isRefund ? "STORNOBELEG · TRAINING" : "TESTBELEG · TRAINING";
    ...
    query("[data-kassenbon-payment]").textContent = isRefund ? "Erstattung: Bar (TRAINING)" : transaction.paymentType === "CASH" ? "Zahlungsart: Bar (TRAINING)" : "Zahlungsart: Karte / unbar (TRAINING)";
  ...
  async function posDeliverySandboxRequest(deliveryId) {
    var token = await apiSessionToken();
    var response = await posFetch("/api/pos-dostava-sandbox", {
      method: "POST",
      headers: { Authorization: "Bearer " + token, "Content-Type": "application/json" },
  ...
  async function posDeliveryEmailRequest(deliveryId) {
    var token = await apiSessionToken();
    var response = await posFetch("/api/pos-dostava-email", {
      method: "POST",
      headers: { Authorization: "Bearer " + token, "Content-Type": "application/json" },
  ...
  async function posDeliveryOpenapiRequest(deliveryId) {
    var token = await apiSessionToken();
    var response = await posFetch("/api/pos?handler=openapi-invoice", {
      method: "POST",
      headers: { Authorization: "Bearer " + token, "Content-Type": "application/json" },
      body: JSON.stringify({ deliveryId: deliveryId, confirmed: true })
    });
    var body = null;
    try { body = await response.json(); } catch (_error) {}
    if (!response.ok) throw new Error(body && body.napaka || "Openapi dostave ni bilo mogoče izvesti.");
    return body;
  }

  async function queueAndRunSandbox(deliveryId) {
    var queued = await withOperationTimeout(backend.client.rpc("pos_queue_invoice_delivery", {
  ...
  function openDeliverySheet(invoice) {
    ...
    deliveryAdjustmentId = null;
    deliveryAdjustmentEntry = null;
    ...
    query("#pos-delivery-title").textContent = "Pošlji račun";
    query("[data-delivery-invoice-reference]").textContent = invoice.number + " · " + formatMoney(invoice.totals.grossCents);
    activateModal(query("[data-delivery-backdrop]"), closeDeliverySheet, query("[data-delivery-close]"));
    syncDeliveryMode();
  ...
  function closeDeliverySheet() {
    ...
    deactivateModal(query("[data-delivery-backdrop]"));
  ...
  async function submitDelivery(event) {
    ...
    if (adjustment && ["cancellation", "credit_note"].indexOf(adjustment.type) !== -1
        && !openapiCapability.financialAdjustmentsEnabled) {
      showToast("Openapi tip 381 je sandbox potrjen, produkcijska aktivacija pa ostaja varno zaklenjena.");
      return;
    }
    var form = event.currentTarget;
    ...
    try {
      if (format === "pdf" || format === "xrechnung_pdf") {
        if (adjustment && !adjustment.documentReady) await ensureAdjustmentDocument(adjustment);
        else if (!adjustment && !invoice.documentReady) await ensureInvoiceDocument(invoice);
      }
      if (format !== "pdf") {
        var sourceDocument = adjustment ? adjustment.einvoiceDocument : invoice.einvoiceDocument;
        var documentStatus = sourceDocument && (sourceDocument.validation_status || sourceDocument.validationStatus);
        if (documentStatus !== "validated") {
          if (adjustment) await ensureAdjustmentEinvoiceDocument(adjustment, true);
          else await ensureEinvoiceDocument(invoice, true);
          sourceDocument = adjustment ? adjustment.einvoiceDocument : invoice.einvoiceDocument;
          documentStatus = sourceDocument && (sourceDocument.validation_status || sourceDocument.validationStatus);
        }
        if (documentStatus !== "validated") throw new Error(adjustment ? "Strukturirani popravek še ni prestal KoSIT validacije." : "Strukturirani račun še ni prestal KoSIT validacije.");
      }
      var deliveryPayload = {
      ...
      if (adjustment) deliveryPayload.p_adjustment_id = adjustment.id;
      else { deliveryPayload.p_invoice_id = invoice.id; deliveryPayload.p_recipient_consent = consent; }
      var result = await withOperationTimeout(backend.client.rpc(adjustment ? "pos_prepare_adjustment_delivery" : "pos_prepare_invoice_delivery", deliveryPayload));
      if (result.error) throw result.error;
      ...
      var openapiEligibleAdjustment = Boolean(adjustment && ["cancellation", "credit_note"].indexOf(adjustment.type) !== -1);
      var openapiDeliveryEnabled = (!adjustment || openapiEligibleAdjustment) && (invoice.draft.customerType === "business" || invoice.draft.customerType === "public") && openapiCapability.sendEnabled;
      var emailDeliveryEnabled = !openapiDeliveryEnabled && form.elements.deliveryChannel.value === "email" && deliveryCapability.sendEnabled;
      var publicSandbox = form.elements.deliveryChannel.value === "email" ? null : publicDeliverySandboxCopy(form.elements.deliveryChannel.value);
      submit.textContent = openapiDeliveryEnabled ? "Oddajam Openapi …" : emailDeliveryEnabled && deliveryCapability.testEnabled ? "Pošiljam test …" : emailDeliveryEnabled && deliveryCapability.liveEnabled ? "Pošiljam …" : publicSandbox ? "Preverjam mock sandbox …" : "Sandbox …";
      var deliveryResult = openapiDeliveryEnabled
        ? await posDeliveryOpenapiRequest(prepared.id)
        : emailDeliveryEnabled ? await posDeliveryEmailRequest(prepared.id)
        : await queueAndRunSandbox(prepared.id);
      deliverySubmitting = false;
      submit.disabled = false;
      submit.textContent = openapiDeliveryEnabled && openapiCapability.sandboxEnabled ? "Pošlji v Openapi sandbox" : emailDeliveryEnabled && deliveryCapability.testEnabled ? "Pošlji test" : emailDeliveryEnabled && deliveryCapability.liveEnabled ? "Pošlji račun" : publicSandbox ? "Zaženi mock sandbox" : "Zaženi sandbox";
      closeDeliverySheet();
      await loadServerState("deliveries");
      activeInvoiceId = invoice.id;
      showView("invoice-detail");
      showToast(openapiDeliveryEnabled && openapiCapability.sandboxEnabled
        ? (adjustment ? "Testni finančni popravek je oddan v Openapi sandbox; prejemnik ga ni prejel." : "Testni račun je oddan v Openapi sandbox; prejemnik ga ni prejel.")
        : openapiDeliveryEnabled && openapiCapability.liveEnabled
        ? (deliveryResult && deliveryResult.sent ? (adjustment ? "Finančni popravek je oddan ponudniku Openapi." : "Račun je oddan ponudniku Openapi.") : (adjustment ? "Finančni popravek čaka na varno Openapi dostavo." : "Račun čaka na varno Openapi dostavo."))
        : emailDeliveryEnabled && deliveryCapability.testEnabled
        ? (adjustment ? "Testni popravek je poslan samo na dovoljeni testni naslov." : "Testni račun je poslan samo na dovoljeni testni naslov.")
        : emailDeliveryEnabled && deliveryCapability.liveEnabled
        ? (deliveryResult && deliveryResult.sent ? (adjustment ? "Popravek je oddan e-poštnemu ponudniku." : "Račun je oddan e-poštnemu ponudniku.") : (adjustment ? "Popravek čaka na varno pošiljanje." : "Račun čaka na varno pošiljanje."))
        : publicSandbox ? publicSandbox.success : "Sandbox preizkus je končan. Nič ni bilo poslano.");
    } catch (error) {
      deliverySubmitting = false;
      submit.disabled = false;
  ...
  async function stripeCheckoutRequest(action, values) {
    ...
    var response = await posFetch("/api/pos-stripe-checkout", {
      method: "POST",
      headers: { Authorization: "Bearer " + token, "Content-Type": "application/json" },
  ...
  async function startStripeCheckout(invoice) {
    ...
    try {
      ...
      var checkoutScope = invoice.id;
      var result;
      if (isActiveStripeAttempt(pending)) {
        result = await stripeCheckoutRequest("resume", { sessionId: pending.checkoutSessionId });
      } else {
        result = await stripeCheckoutRequest("create", {
          invoiceId: invoice.id,
          requestId: operationRequestId("stripe-checkout", checkoutScope)
        });
      }
      clearOperationRequestId("stripe-checkout", checkoutScope);
  ...
  async function abandonStripeCheckout(invoice) {
    var payment = latestStripePayment(invoice);
    if (!invoice || !invoice.isTest || !invoice.serverStored || !backend.ready || !isActiveStripeAttempt(payment)) {
      showToast("Ni aktivnega Stripe TEST poskusa.");
      return;
    }
    var button = query("[data-stripe-abandon]");
    button.disabled = true;
    query("span", button).textContent = "Končujem Stripe poskus …";
    try {
      var result = await stripeCheckoutRequest("cancel", { sessionId: payment.checkoutSessionId });
      await loadServerState("payments");
      if (result.payment && result.payment.status === "succeeded") {
        showToast("Stripe je plačilo že potrdil.");
      } else {
        showToast("Stripe poskus je varno končan.");
      }
    } catch (error) {
      renderStripePayment(invoice);
      showToast(error && error.message || "Stripe poskusa ni bilo mogoče varno končati.");
    }
  }
    ...
    try {
      ...
      if (returnState.state === "cancelled") {
        ...
        var cancelSessionId = String(returnState.sessionId || "").indexOf("cs_test_") === 0
          ...
          : isActiveStripeAttempt(pendingPayment) ? pendingPayment.checkoutSessionId : "";
  ...
  async function posPdfRequest(invoiceId, mode) {
    var token = await apiSessionToken();
    var response = await posFetch("/api/pos-racun-pdf?invoiceId=" + encodeURIComponent(invoiceId) + "&mode=" + encodeURIComponent(mode || "download"), {
      method: mode === "metadata" ? "POST" : "GET",
      headers: { Authorization: "Bearer " + token }
    }, POS_DOCUMENT_TIMEOUT_MS);
    if (!response.ok) {
      var body = null;
      try { body = await response.json(); } catch (_error) {}
  ...
  async function posEinvoiceRequest(invoiceId, mode) {
    ...
    var response = await posFetch("/api/pos-racun-xrechnung?invoiceId=" + encodeURIComponent(invoiceId) + "&mode=" + encodeURIComponent(action), {
  ...
  async function posEinvoicePreflightRequest(draftId) {
    var token = await apiSessionToken();
    var response = await posFetch("/api/pos-racun-xrechnung?draftId=" + encodeURIComponent(draftId) + "&mode=preflight", {
      method: "POST",
      headers: { Authorization: "Bearer " + token }
    }, POS_DOCUMENT_TIMEOUT_MS);
    var body = null;
    try { body = await response.json(); } catch (_error) {}
    if (!response.ok || !body || !body.ok || !body.preflight) {
      throw new Error(body && body.napaka || "KoSIT predizdajna validacija ni uspela.");
    }
  ...
  async function adjustmentPdfRequest(adjustmentId, mode) {
    var token = await apiSessionToken();
    var response = await posFetch("/api/pos-racun-korekcija?adjustmentId=" + encodeURIComponent(adjustmentId) + "&mode=" + encodeURIComponent(mode || "download"), {
      method: mode === "metadata" ? "POST" : "GET",
      headers: { Authorization: "Bearer " + token }
    }, POS_DOCUMENT_TIMEOUT_MS);
    if (!response.ok) {
  ...
  function openAdjustmentSheet(invoice, initialType) {
    ...
    activateModal(query("[data-adjustment-backdrop]"), closeAdjustmentSheet, query("[name=reason]", form));
    syncAdjustmentMode();
  ...
  function closeAdjustmentSheet() {
    ...
    deactivateModal(query("[data-adjustment-backdrop]"));
  ...
  async function submitAdjustment(event) {
    ...
    var adjustmentRequestScope = invoice.id + ":" + operationScopeHash(JSON.stringify([type, reason, changes]));
    try {
      var result = await withOperationTimeout(backend.client.rpc("pos_create_invoice_adjustment", {
        p_invoice_id: invoice.id,
        p_request_key: operationRequestId("invoice-adjustment", adjustmentRequestScope),
      ...
      if (result.error) throw result.error;
      clearOperationRequestId("invoice-adjustment", adjustmentRequestScope);
  ...
  function readBonitetaOfferTransfer() {
    try {
      return normalizeBonitetaContext(JSON.parse(global.sessionStorage.getItem(BONITETA_OFFER_TRANSFER_KEY) || "null"), true);
    } catch (_error) {
      return null;
    }
  }

  function offerDraftFromBoniteta(context) {
    var draft = defaultDraft(state.profile);
    var companies = Array.isArray(context.companies) ? context.companies : [context];
    draft.workflowMode = "offer";
    draft.offerValidDays = "14";
    draft.customerType = "business";
    if (companies.length === 1) {
      draft.customerName = context.company.name;
      draft.customerStreet = context.company.street;
      draft.customerPostalCode = context.company.postalCode;
      draft.customerCity = context.company.city;
    }
    draft.bonitetaContext = context;
    return draft;
  }

  function importBonitetaOfferTransfer() {
    var cleanUrl = new URL(global.location.href);
    cleanUrl.searchParams.delete("from");
    global.history.replaceState(null, "", cleanUrl.pathname + cleanUrl.search + cleanUrl.hash);
    var context = readBonitetaOfferTransfer();
    if (!context) {
      try { global.sessionStorage.removeItem(BONITETA_OFFER_TRANSFER_KEY); } catch (_error) {}
      showToast("Prenos iz bonitetne preverbe ni več na voljo. Podjetje prenesite ponovno.");
      return;
    }
    var applyTransfer = function () {
      state.draft = offerDraftFromBoniteta(context);
      currentStep = 1;
      persist();
      try { global.sessionStorage.removeItem(BONITETA_OFFER_TRANSFER_KEY); } catch (_error) {}
      showView("invoice");
      showToast(context.companies.length > 1 ? "Primerjava obeh podjetij je prenesena. Izberite prejemnika ponudbe." : "Preverjeno podjetje je preneseno v osnutek ponudbe.");
    };
    if (!state.draft) {
      applyTransfer();
      return;
    }
    openDialog(context.companies.length > 1 ? "Prenesti obe podjetji v novo ponudbo?" : "Prenesti podjetje v novo ponudbo?", context.companies.length > 1 ? "Trenutni osnutek bo zamenjan. Primerjalni podatki obeh podjetij bodo dodani, prejemnika pa boste izbrali ročno." : "Trenutni osnutek bo zamenjan. Preverjeni podatki podjetja in izbrana priporočila bodo dodani novi ponudbi.", {
      confirmText: "Prenesi",
  ...
  function renderEditor() {
    ...
    var boniteta = offerMode ? normalizeBonitetaContext(state.draft.bonitetaContext) : null;
    var bonitetaBanner = query("[data-boniteta-transfer]");
    bonitetaBanner.hidden = !boniteta;
    if (boniteta) {
      var prenesenaPodjetja = Array.isArray(boniteta.companies) ? boniteta.companies : [boniteta];
      query("[data-boniteta-transfer-company]").textContent = prenesenaPodjetja.length > 1 ? prenesenaPodjetja.map(function (item) { return item.company.name; }).join(" · ") : boniteta.company.name + " · " + [boniteta.company.street, boniteta.company.postalCode, boniteta.company.city].join(", ");
      var selected650f = prenesenaPodjetja.some(function (item) { return item.recommendations.some(function (entry) { return entry.key === "payment_security_650f" && entry.selected; }); });
      var checkedOn = boniteta.verification.checkedAt && Date.parse(boniteta.verification.checkedAt) ? formatDate(berlinDateKey(boniteta.verification.checkedAt)) : "datum ni naveden";
      query("[data-boniteta-transfer-summary]").textContent = (prenesenaPodjetja.length > 1 ? "Prenesen je primerjalni povzetek obeh podjetij" : "Boniteta preverjena " + checkedOn) + " · " + (selected650f ? "§650f je označen za preverjanje pri ponudbi." : "Brez izbranega dodatnega priporočila.");
    }
  ...
  function openAdjustmentDeliverySheet(invoice, adjustment) {
    var sourceInvoice = adjustment && adjustment.originalInvoiceId ? findInvoice(adjustment.originalInvoiceId) : invoice;
    if (!sourceInvoice || !adjustment || ["correction", "cancellation", "credit_note"].indexOf(adjustment.type) === -1) {
      showToast("Strukturirani popravek ni na voljo za dostavo."); return;
    }
    if (!sourceInvoice.serverStored || !backend.ready) { showToast("Pošiljanje potrebuje varno shranjen popravek."); return; }
    deliveryInvoiceId = sourceInvoice.id;
    deliveryAdjustmentId = adjustment.id;
    deliveryAdjustmentEntry = adjustment;
    deliveryRequestKey = randomUuid();
    var form = query("#pos-delivery-form");
    form.reset();
    var isPublic = sourceInvoice.draft.customerType === "public";
    form.elements.deliveryChannel.value = isPublic ? "ozg_re" : "email";
    form.elements.deliveryFormat.value = isPublic ? "xrechnung" : "xrechnung_pdf";
    form.elements.deliveryRecipient.value = sourceInvoice.draft.customerEmail || "";
    form.elements.deliveryRoutingReference.value = sourceInvoice.draft.leitwegId || "";
    var adjustmentTitle = adjustment.type === "cancellation" ? "Stornorechnung " : adjustment.type === "credit_note" ? "Gutschrift " : "Rechnungsberichtigung ";
    form.elements.deliverySubject.value = adjustmentTitle + adjustment.number;
    form.elements.deliveryMessage.value = "Guten Tag,\n\nanbei erhalten Sie die " + adjustmentTitle + adjustment.number + " zur Rechnung " + sourceInvoice.number + ".\n\nMit freundlichen Grüßen\n" + (state.profile.legalName || "");
    query("#pos-delivery-title").textContent = adjustment.type === "cancellation" ? "Pošlji Storno" : adjustment.type === "credit_note" ? "Pošlji dobropis" : "Pošlji popravek";
    query("[data-delivery-invoice-reference]").textContent = adjustment.number + " · original " + sourceInvoice.number;
    activateModal(query("[data-delivery-backdrop]"), closeDeliverySheet, query("[data-delivery-close]"));
    syncDeliveryMode();
    loadDeliveryCapability().then(function () { if (deliveryAdjustmentId === adjustment.id) syncDeliveryMode(); });
    ensureAdjustmentEinvoiceDocument(adjustment, true).then(function () {
      if (deliveryAdjustmentId === adjustment.id) syncDeliveryMode();
    }).catch(function (error) {
      if (deliveryAdjustmentId === adjustment.id) { syncDeliveryMode(); showToast(error.message || "Strukturirani popravek še ni pripravljen."); }
    });
    fitAllText();
  }
  ...
  function markInvalid(step, errors) {
    queryAll(".is-invalid, [aria-invalid=\"true\"]").forEach(function (field) {
  ...
  function setStep(step, validateCurrent) {
    ...
    queryAll("[data-steps] li").forEach(function (li, index) {
      ...
      var button = query("button[data-step]", li);
  ...
  function renderPreview() {
    ...
    query("[data-issue-invoice]").textContent = invoiceIssuing ? "Izdajam račun …" : offerMode ? (draft.workflowContext && draft.workflowContext.workOrderId ? "Posodobi ponudbo" : "Ustvari ponudbo") : invoice.isTest ? "Ustvari testni račun" : "Pravno izdaj račun";
    query("[data-issue-invoice]").disabled = errors.length > 0 || invoiceIssuing;
    if (offerMode) {
  ...
  async function issueOnServer() {
    ...
    var draftId = await saveDraftToServer({ allowIssuedRetry: true });
    var needsKositPreflight = profileReadiness(state.profile).live &&
      (state.draft.customerType === "public" || String(state.draft.issueDate || "") >= "2028-01-01");
    if (needsKositPreflight) await posEinvoicePreflightRequest(draftId);
    var replacement = normalizeReplacementContext(state.draft);
    ...
    var result = await withOperationTimeout(backend.client.rpc(rpcName, rpcPayload));
    if (result.error) throw result.error;
  ...
  function issueInvoice() {
    ...
    var readiness = profileReadiness(state.profile);
    var live = productionReady();
    var kositRequired = live && (state.draft.customerType === "public" || String(state.draft.issueDate || "") >= "2028-01-01");
    ...
    openDialog(
      ...
      replacement
        ...
        : live ? "Po izdaji vsebine ne bo mogoče spreminjati. Morebitni popravek bo nov dokument." + (kositRequired ? " Podatki prejemnika, postavke in zneski bodo pred izdajo poslani nastavljenemu KoSIT validatorju." : "") : "Dokument bo jasno označen kot TESTRECHNUNG in ni primeren za pravo poslovno uporabo.",
      {
        confirmText: replacement ? "Izdaj nadomestni račun" : live ? "Izdaj račun" : "Ustvari test",
        onConfirm: async function () {
          if (invoiceIssuing) return;
          invoiceIssuing = true;
          var issueButton = query("[data-issue-invoice]");
          issueButton.disabled = true;
          issueButton.setAttribute("aria-busy", "true");
          issueButton.textContent = "Izdajam račun …";
          try {
  ...
  function issueOffer() {
    ...
    openDialog(existingId ? "Posodobiti ponudbo?" : "Ustvariti ponudbo?", existingId ? "Spremembe bodo shranjene v osnutek. Poslana ponudba ostane nespremenljiva." : "Ponudba dobi zaporedno številko. Zaklene se šele, ko jo označite kot poslano.", {
      ...
      onConfirm: async function () {
        try {
          var result = await withOperationTimeout(backend.client.rpc("pos_save_work_order", { p_work_order_id: existingId, p_payload: workOrderPayloadFromDraft(state.draft) }).single());
          if (result.error) throw result.error;
          state.draft = null;
          persist();
          await loadServerState("invoices");
  ...
  function copyPaymentForInvoice(invoice) {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      Promise.resolve()
        .then(function () { return navigator.clipboard.writeText(text); })
        .then(function () { showToast("Plačilni podatki so kopirani."); })
        .catch(function () { openDialog("Plačilni podatki", text, { cancel: false }); });
    }
  ...
  function requestPayment(id) {
    ...
    var paymentRequestScope = invoice.id + ":" + outstandingCents;
      ...
      onConfirm: async function () {
        try {
          ...
          if (invoice.serverStored) {
            ...
            var result = await withOperationTimeout(backend.client.rpc("pos_record_manual_payment", {
              p_invoice_id: invoice.id,
              p_request_key: operationRequestId("manual-payment", paymentRequestScope),
              p_confirmed: true
            }));
            if (result.error) throw result.error;
          ...
          clearOperationRequestId("manual-payment", paymentRequestScope);
          persist();
  ...
  async function posOfferPdfRequest(workOrderId, mode) {
    var token = await apiSessionToken();
    var action = mode || "download";
    var response = await posFetch("/api/pos-angebot-pdf?workOrderId=" + encodeURIComponent(workOrderId) + "&mode=" + encodeURIComponent(action), {
  ...
  async function adjustmentEinvoiceRequest(adjustmentId, mode) {
    var token = await apiSessionToken();
    var response = await posFetch("/api/pos-racun-korekcija-xrechnung?adjustmentId=" + encodeURIComponent(adjustmentId) + "&mode=" + encodeURIComponent(mode || "download"), {
  ...
  async function ensureAdjustmentEinvoiceDocument(adjustment, forceValidation) {
    ...
    if (!adjustment.einvoiceDocumentReady || status !== "validated" || forceValidation && status !== "validated") {
    ...
    persist();
    return adjustment.einvoiceDocument;
  }

  async function downloadAdjustmentEinvoice(adjustment) {
    await ensureAdjustmentEinvoiceDocument(adjustment, true);
  ...
  async function posContractConfirmationPdfRequest(workOrderId, mode) {
    var token = await apiSessionToken();
    var action = mode || "download";
    var response = await posFetch("/api/pos-pogodba-pdf?workOrderId=" + encodeURIComponent(workOrderId) + "&mode=" + encodeURIComponent(action), {
      method: action === "metadata" ? "POST" : "GET",
      headers: { Authorization: "Bearer " + token }
    }, POS_DOCUMENT_TIMEOUT_MS);
    if (!response.ok) {
  ...
  async function loadFinapiBankStatus(showFeedback) {
    ...
    try {
      var token = await apiSessionToken();
      var response = await posFetch("/api/pos-finapi", { method: "GET", headers: { Authorization: "Bearer " + token } });
      var body = null;
      try { body = await response.json(); } catch (_error) {}
  ...
  async function syncFinapiBank() {
    ...
    try {
      ...
      var response = await posFetch("/api/pos-finapi", {
        method: "POST",
        headers: { Authorization: "Bearer " + token, "Content-Type": "application/json" },
  ...
  function closeBankSheet() {
    deactivateModal(query("[data-bank-backdrop]"));
  }
  ...
  function openBankSheet() {
    renderBankSheet();
    activateModal(query("[data-bank-backdrop]"), closeBankSheet, query("[data-bank-close]"));
    ...
    openDialog("Potrditi bančno plačilo?", formatMoney(transaction.amountCents) + " bo povezano z računom " + invoice.number + ". Bančni zapis in račun ostaneta sledljiva.", {
      ...
      onConfirm: async function () {
        try {
          ...
          var result = await withOperationTimeout(backend.client.rpc("pos_confirm_bank_transaction", {
  ...
  function importBankFile(file) {
    ...
    reader.onload = async function () {
      try {
        ...
        var result = await withOperationTimeout(backend.client.rpc("pos_import_bank_transactions", {
  ...
  function openDatevSheet() {
    ...
    activateModal(query("[data-datev-backdrop]"), closeDatevSheet, query("[data-datev-close]"));
    renderDatevCloud();
    loadDatevCloudStatus(false);
  }

  function closeDatevSheet() {
    deactivateModal(query("[data-datev-backdrop]"));
  }
    ...
    openDialog(
      ...
      "Vnesite celotni ali delni znesek. Pravi denar se ne premakne; podpisani webhook bo posodobil plačilno sled.",
      {
        ...
        onConfirm: async function (value) {
          ...
          var refundScope = payment.id + ":" + amountCents;
          ...
          try {
            await stripeCheckoutRequest("refund", {
              ...
              confirmed: true,
              requestId: operationRequestId("stripe-refund", refundScope)
            });
            ...
            for (var attempt = 0; attempt < 7; attempt += 1) {
              ...
              if (refreshedPayment && integer(refreshedPayment.refundedCents, 0) >= integer(payment.refundedCents, 0) + amountCents) {
            ...
            if (webhookConfirmed) {
              clearOperationRequestId("stripe-refund", refundScope);
              showToast("Stripe TEST povračilo je potrjeno s podpisanim webhookom.");
            } else {
              showToast("Povračilo je bilo sprejeto, vendar podpisani webhook še ni potrjen. Stanje osvežite pozneje.");
            }
          } catch (error) {
  ...
  async function datevCloudRequest(action, values, method) {
    ...
    var response = await posFetch(target, options, POS_DOCUMENT_TIMEOUT_MS);
    var result = null;
    try { result = await response.json(); } catch (_error) {}
  ...
  function updateDatevCloudCapability(result) {
    ...
    datevCloudCapability.consultantNumber = Number.isInteger(Number(connection.consultantNumber)) && Number(connection.consultantNumber) > 0 ? Number(connection.consultantNumber) : null;
    datevCloudCapability.clientNumber = Number.isInteger(Number(connection.clientNumber)) && Number(connection.clientNumber) > 0 ? Number(connection.clientNumber) : null;
  ...
  function renderDatevCloud() {
    ...
    var connectedIdentifiers = [];
    if (datevCloudCapability.consultantNumber) connectedIdentifiers.push("Berater " + datevCloudCapability.consultantNumber);
    if (datevCloudCapability.clientNumber) connectedIdentifiers.push("Mandant " + datevCloudCapability.clientNumber);
    ...
    query("[data-datev-cloud-status]").textContent = datevCloudCapability.loading ? "Preverjam varno povezavo …"
      : datevCloudCapability.working ? "Varno izvajam DATEV opravilo …"
        : datevCloudCapability.lastError || (datevCloudCapability.connected ? connectedIdentifiers.length ? "Povezano · " + connectedIdentifiers.join(" · ") : "Povezano · pripravljeno za prenos" : datevCloudCapability.configured ? "Pripravljeno za povezavo" : "DATEV OAuth podatki še niso izdani");
    ...
    query("[data-datev-cloud-latest]").textContent = latest
      ? (latest.status === "succeeded" ? "Uspešno: " : latest.status === "processing" ? "DATEV še obdeluje: " : latest.status === "failed" ? "Neuspešno: " : "Priprava: ") + latest.period + " · " + Number(latest.documentCount || 0) + " dokumentov · " + Number(latest.bookingCount || 0) + " knjižb" + (latest.status === "failed" && latest.errorMessage ? " · " + String(latest.errorMessage) : "")
  ...
  async function transferDatevCloud() {
    ...
    datevCloudCapability.lastError = "";
    renderDatevCloud();
    var transferAction = mockTest ? "test-transfer" : "transfer";
    var transferPeriod = query("[name=datevPeriod]").value;
    var transferScope = transferAction + ":" + transferPeriod;
    try {
      var response = await datevCloudRequest(transferAction, {
        period: transferPeriod,
        requestId: operationRequestId("datev-transfer", transferScope)
      });
      clearOperationRequestId("datev-transfer", transferScope);
  ...
  function fitInput(field) {
    ...
    var displayValue = isSelect && field.selectedOptions && field.selectedOptions[0] ? field.selectedOptions[0].textContent : field.value;
    field.style.setProperty("font-size", max + "px", "important");
    if (!displayValue) return;
    var arrowReserve = isSelect ? 28 : 4;
    var available = field.clientWidth - parseFloat(style.paddingLeft) - parseFloat(style.paddingRight) - arrowReserve;
    ...
    while (size > min) {
      ...
      if (context.measureText(displayValue).width <= available) break;
      size -= .5;
    }
  ...
  function bindEvents() {
    ...
    query("#pos-invoice-form").addEventListener("input", function (event) {
    ...
    query("#pos-profile-form").addEventListener("input", function (event) {
      ...
      if (event.target.name === "taxStatus") syncProfileTaxFields();
    });
    ...
    query("[data-procedure-documentation]").addEventListener("click", downloadProcedureDocumentation);
    query("[data-procedure-versions]").addEventListener("click", function (event) {
      var button = event.target.closest("[data-procedure-version-id]");
      if (button) downloadProcedureDocumentation(button.getAttribute("data-procedure-version-id"));
    });
    ...
    query("[data-stripe-abandon]").addEventListener("click", function () { var invoice = findInvoice(activeInvoiceId); if (invoice) abandonStripeCheckout(invoice); });
    ...
    query("[data-cash-payment]").addEventListener("click", function () { var invoice = findInvoice(activeInvoiceId); if (invoice) requestLocalCashPayment(invoice); });
    query("[data-cash-refund]").addEventListener("click", function () { var invoice = findInvoice(activeInvoiceId); if (invoice) refundLocalCashPayment(invoice); });
    query("[data-cash-receipt]").addEventListener("click", function () { var invoice = findInvoice(activeInvoiceId); if (invoice) showLocalCashReceipt(invoice); });
    query("[data-cash-deposit]").addEventListener("click", function () { recordLocalCashMovement("DEPOSIT"); });
    query("[data-cash-withdrawal]").addEventListener("click", function () { recordLocalCashMovement("WITHDRAWAL"); });
    query("[data-dsfinvk-export]").addEventListener("click", downloadDsfinvk);
    ...
    if (document.fonts && document.fonts.ready) {
      Promise.resolve(document.fonts.ready)
        .then(fitAllText)
        .catch(function () { global.setTimeout(fitAllText, 0); });
    }
  ...
  global.UJPoskusiNotranjiKorakNazaj = function () {
    if (!query("[data-dialog-backdrop]").hidden) { closeDialog(false); return true; }
  ...
  function init() {
    ...
    var bonitetaReturn = returnParams.get("from") === "boniteta";
    var stripeSessionId = returnParams.get("stripe_session_id");
    var stripeInvoiceId = returnParams.get("invoice_id");
    var initialLoad = loadServerState().catch(function (error) {
      backend.ready = false;
      try { backendMessage(databaseErrorMessage(error), "error"); }
      catch (_renderError) {}
    });
    if (finapiReturn) {
    ...
    if (bonitetaReturn) initialLoad.then(importBonitetaOfferTransfer);
  }
