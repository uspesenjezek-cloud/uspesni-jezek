...
var aiSdk = require("./atena-ai-sdk");
...
var AGREEMENT_CLARIFICATION_BY_CODE = Object.freeze({
  agreement_kind: "Kaj je dolžnik dejansko sprejel ali povedal?",
  accepted_or_proposed: "Ali je bil ta predlog že dogovorjen ali je bil samo predlagan?",
  promise_or_refusal: "Ali je dolžnik plačilo obljubil ali ga je zavrnil?",
});

var AGREEMENT_RESPONSE_ZOD_SCHEMA = structuredContracts.agreementProposalSchema(AGREEMENT_CATALOG.map(function (item) { return item.id; }), lunaPolicy.MAX_STRUCTURED_ITEMS, Object.keys(AGREEMENT_CLARIFICATION_BY_CODE));
var RESPONSE_SCHEMA = structuredContracts.compactJsonSchema(z.toJSONSchema(AGREEMENT_RESPONSE_ZOD_SCHEMA));
...
function requestBody(text, context, userId) {
  ...
  body.instructions = "HARD COMPLETE-CATALOG ORDER: FIRST read every entry in catalog.guide, including useWhen, doNotUseWhen, examples, languageProfile, numeric cardId, field IDs and allowed value IDs. Choose and return cardId yourself; never return a free-form card type. " + catalogContract.semanticInstructions() + " " + lunaPolicy.semanticAuthorityInstructions() + " HARD OUTPUT INVARIANTS: never return agreements and question together. Every agreement evidence must be a non-empty exact contiguous quote copied verbatim from sourceText. For a clarification, top-level evidence must be such a quote. Never write a free-form clarification question: set question to exactly one closed ambiguity code: agreement_kind, accepted_or_proposed, or promise_or_refusal. " + body.instructions + " The local adapter maps that numeric cardId to the existing card and maps the closed ambiguity code to approved local question text; it does not make or correct a semantic choice.";
  return body;
}
...
function materializeLunaProposal(proposal, context, source) {
  ...
  var parsed = AGREEMENT_RESPONSE_ZOD_SCHEMA.safeParse(proposal);
  if (!parsed.success || !parsed.data.agreements.length) return null;
  proposal = parsed.data;
  var document = structuredContracts.createDocument(source);
  ...
  for (var index = 0; index < proposal.agreements.length; index += 1) {
    ...
    var occurredDate = item.occurredDate;
    var promisedDate = item.promisedDate;
    var channel = item.communicationChannel == null ? "unknown" : item.communicationChannel;
    var fields = catalogItem.fields.slice();
    var clauseId = "clause-" + (index + 1);
    var evidenceSpan = structuredContracts.createEvidenceSpan(document, item.evidence, clauseId);
    if (!evidenceSpan || !structuredContracts.verifyEvidenceSpan(document, evidenceSpan)) return null;
    candidates.push({
      ...
      evidence: { clauseId: clauseId, sourceSpan: structuredContracts.legacySourceSpan(evidenceSpan), span: evidenceSpan, explicit: false, reason: "luna_agreement_catalog_adapter" },
...
function clarificationResult(proposal, context, source) {
  ...
  var parsed = AGREEMENT_RESPONSE_ZOD_SCHEMA.safeParse(proposal);
  if (!parsed.success || parsed.data.agreements.length || parsed.data.question === null || parsed.data.evidence === null) return null;
  proposal = parsed.data;
  var document = structuredContracts.createDocument(source);
  var evidenceSpan = structuredContracts.createEvidenceSpan(document, proposal.evidence, "clause-1");
  if (!evidenceSpan || !structuredContracts.verifyEvidenceSpan(document, evidenceSpan)) return null;
  var questionText = AGREEMENT_CLARIFICATION_BY_CODE[proposal.question];
  ...
  return {
    ...
    clarification: exhausted ? null : { code: proposal.question, question: questionText, clauseId: "clause-1", evidenceSpan: evidenceSpan, round: nextRound, maxRounds: MAX_CLARIFICATION_ROUNDS },
...
async function analyze(text, context, options) {
  ...
  try {
    ...
    var transport = await aiSdk.generateStructured({
      apiKey: apiKey, model: request.model, instructions: request.instructions, input: request.input,
      schema: AGREEMENT_RESPONSE_ZOD_SCHEMA, schemaName: "future_agreement_proposal_v7",
      maxOutputTokens: request.max_output_tokens, reasoningEffort: request.reasoning && request.reasoning.effort,
      safetyIdentifier: request.safety_identifier, fetchImpl: options.fetchImpl, timeoutMs: options.timeoutMs,
      requestBytes: Buffer.byteLength(requestJson, "utf8"), legacyRequestBody: requestJson,
      maxAttempts: options.maxAttempts, sleepImpl: options.sleepImpl, randomImpl: options.randomImpl,
    });
    payload = { usage: { input_tokens: transport.usage.inputTokens, output_tokens: transport.usage.outputTokens, total_tokens: transport.usage.totalTokens } };
    payload._atenaTransport = { attempts: transport.attempts, calls: transport.calls, elapsedMs: transport.elapsedMs, adapter: transport.adapterVersion };
    var proposal = transport.output;
    ...
    if (error && error.code === "ATENA_AI_SDK_INVALID_OUTPUT") throw aiError("Lunin odgovor ni skladen z dogovornim katalogom.", "LUNA_INVALID_AGREEMENT_PLAN");
...
module.exports = {
  ...
  MAX_CLARIFICATION_ROUNDS: MAX_CLARIFICATION_ROUNDS, RESPONSE_SCHEMA: RESPONSE_SCHEMA, RESPONSE_ZOD_SCHEMA: AGREEMENT_RESPONSE_ZOD_SCHEMA,
  CLARIFICATION_CODES: Object.freeze(Object.keys(AGREEMENT_CLARIFICATION_BY_CODE)),
