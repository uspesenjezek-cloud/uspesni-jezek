...
const GENERATOR_VERSION = "uj-pos-pdf-9";
...
function drawHeaderSeller(page, value, font, options) {
  const settings = options || {};
  const clean = safeText(value || "Unternehmen");
  const maxWidth = Number(settings.maxWidth) || 278;
  const preferred = Number(settings.preferred) || 15;
  const minimum = Number(settings.minimum) || 8.5;
  const size = fitSize(clean, font, maxWidth, preferred, minimum);
  const lines = wrap(clean, font, size, maxWidth);
  const lineHeight = size + (Number(settings.lineHeightExtra) || 1.5);
  const baseline = Number(settings.baseline) || 0;
  const firstY = baseline + (lines.length - 1) * lineHeight / 2;
  lines.forEach((line, index) => page.drawText(line, {
    x: Number(settings.x) || PAGE.margin,
    y: firstY - index * lineHeight,
    font,
    size,
    color: settings.color || COLORS.white
  }));
  return { lines, size };
}
...
function sellerLegalDisclosureLines(seller) {
  ...
  const registered = REGISTERED_LEGAL_FORMS.includes(form);
  const lines = [];
  ...
  if (representative) {
    const role = form === "GmbH" || form === "UG (haftungsbeschränkt)" || form === "GmbH & Co. KG" ? "Geschäftsführung: "
      : form === "AG" || form === "eG" ? "Vorstand: "
        : form === "Einzelunternehmen" || form === "e.K." ? "Inhaber/in: " : "Vertretungsberechtigte: ";
    lines.push(role + representative);
  }
  return lines;
}
...
function sellerForInvoice(invoice) {
  ...
  const registered = REGISTERED_LEGAL_FORMS.includes(safeText(source.legalForm));
...
async function ustvariRacunPdf(invoice) {
  ...
  function addPage(continuation) {
    ...
    drawHeaderSeller(page, seller.legalName, bold, {
      ...
      preferred: 16, minimum: 8.5, color: COLORS.white
    });
  ...
  const sellerLines = [seller.legalName].concat(sellerAddress).flatMap((line) => wrap(line, regular, 9, 250));
  ...
  y = drawLines(page, sellerLines, PAGE.margin, y - 15, { font: regular, size: 9, lineHeight: 12 });
  ...
  const customerLines = customer.flatMap((line) => wrap(line, regular, 10, 250));
  page.drawText("Rechnung an", { x: PAGE.margin, y, font: bold, size: 7.5, color: COLORS.muted });
  y = drawLines(page, customerLines, PAGE.margin, y - 15, { font: customer.length ? regular : bold, size: 10, lineHeight: 13 }) - 8;
  ...
  const paymentDetailLines = paymentLines.slice(1).flatMap((line) => wrap(line, regular, 8.5, 265));
  const taxIdentity = taxIdentityText(seller) || (sellerInfo.testTax ? "Steuerdaten: TEST - nicht vorhanden" : "");
  const paymentMetaLines = [taxIdentity, seller.businessEmail].filter(Boolean)
    .flatMap((line) => wrap(line, regular, 7.6, 215));
  ensureSpace(Math.max(86, 28 + Math.max(paymentDetailLines.length, paymentMetaLines.length) * 12));
  page.drawText(paymentLines[0], { x: PAGE.margin, y, font: bold, size: 9.5, color: COLORS.tealDark });
  drawLines(page, paymentDetailLines, PAGE.margin, y - 16, { font: regular, size: 8.5, lineHeight: 12, color: COLORS.ink });
  paymentMetaLines.forEach((line, index) => {
    rightText(page, line, PAGE.width - PAGE.margin, y - 16 - index * 12, regular, 7.6, COLORS.muted);
  });
...
module.exports = { GENERATOR_VERSION, safeText, money, dateDE, wrap, fitSize, drawHeaderSeller, taxGroups, priceMode, lineDisplayAmount, taxIdentityText, sellerLegalDisclosureLines, sellerForInvoice, propertyRetentionNote, constructionWithholdingNote, paymentInstructions, embedUnicodeFonts, ustvariRacunPdf };
