...
function compactPendingBinding(official, primaryRun, detailsRun) {
  var officialCompany = official && official.status === "found" && official.company ? official.company : official || {};
  return {
    companyId: clean(officialCompany.company_id || officialCompany.companyId, 140),
    legalName: clean(officialCompany.name, 240),
    register: companyRegister(officialCompany),
    registerCourt: clean(officialCompany.register_court || officialCompany.registerCourt, 140),
    primaryRunId: clean(primaryRun && primaryRun.runId, 80),
    detailsRunId: clean(detailsRun && detailsRun.runId, 80),
  };
}
...
function signPending(userId, official, primaryRun, detailsRun, now) {
  var binding = compactPendingBinding(official, primaryRun, detailsRun);
  if (!clean(userId, 80) || !binding.companyId || !binding.legalName || !binding.register || !binding.primaryRunId) {
    throw Object.assign(new Error("Ozadnega North Data preverjanja ni mogoče varno povezati s podjetjem."), {
      status: 409,
      code: "NORTHDATA_PENDING_BINDING_INCOMPLETE",
    });
  }
  var payload = { v: 2, uid: clean(userId, 80), exp: Number(now || Date.now()) + TTL_MS, binding: binding };
  var encoded = Buffer.from(JSON.stringify(payload)).toString("base64url");
  return encoded + "." + crypto.createHmac("sha256", secret()).update(encoded).digest("base64url");
}

function verifyPending(token, userId, now) {
  var parts = clean(token, 8000).split(".");
  if (parts.length !== 2 || !parts[0] || !parts[1]) return null;
  var expected = crypto.createHmac("sha256", secret()).update(parts[0]).digest();
  var supplied;
  ...
  var payload;
  try { payload = JSON.parse(Buffer.from(parts[0], "base64url").toString("utf8")); } catch (_) { return null; }
  if (!payload || payload.v !== 2 || Number(payload.exp) < Number(now || Date.now()) || clean(payload.uid, 80) !== clean(userId, 80)) return null;
  var binding = payload.binding || {};
  return binding.companyId && binding.legalName && binding.register && binding.primaryRunId
    ? { exp: Number(payload.exp), binding: binding }
    : null;
}

function matchesPending(payload, official, primaryRun, detailsRun) {
  if (!payload || !payload.binding) return false;
  var actual = compactPendingBinding(official, primaryRun, detailsRun);
  return Object.keys(actual).every(function (key) { return actual[key] === clean(payload.binding[key], key === "legalName" ? 240 : 140); });
}

module.exports = {
  TTL_MS: TTL_MS,
  compactBinding: compactBinding,
  compactPendingBinding: compactPendingBinding,
  sign: sign,
  verify: verify,
  matches: matches,
