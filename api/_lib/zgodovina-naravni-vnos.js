...
var RESPONSE_ZOD_SCHEMA = structuredContracts.historyProposalSchema(Object.values(CARD_ID_BY_TYPE), Object.values(FIELD_ID_BY_NAME), MAX_EVENTS);
...
function normalizeEvent(raw, index) {
  ...
  return {
    ...
    occurredDatePrecision: ["exact", "month", "year"].includes(raw && raw.occurredDatePrecision) ? raw.occurredDatePrecision : null,
    occurredDateKnownYearMonth: /^\d{4}-\d{2}$/.test(String(raw && raw.occurredDateKnownYearMonth || "")) ? raw.occurredDateKnownYearMonth : null,
    occurredDateKnownYear: /^\d{4}$/.test(String(raw && raw.occurredDateKnownYear || "")) ? raw.occurredDateKnownYear : null,
...
function resolveReferenceDateRelations(candidates, referenceDate) {
  ...
  candidates.forEach(function (candidate) {
    ...
    if (!derived) return;
    if (relation.dayOfMonth == null && candidate.occurredDatePrecision === "month") {
      candidate.occurredDate = null;
      candidate.occurredDateKnownYearMonth = derived.slice(0, 7);
      candidate.occurredDateKnownYear = derived.slice(0, 4);
      candidate.occurredDateDerived = false;
      candidate.occurredDateManualOverride = false;
      candidate.occurredDateDerivedFrom = "reference_date_partial_month";
      return;
    }
    if (relation.dayOfMonth == null && candidate.occurredDatePrecision === "year") {
      candidate.occurredDate = null;
      candidate.occurredDateKnownYearMonth = null;
      candidate.occurredDateKnownYear = derived.slice(0, 4);
      candidate.occurredDateDerived = false;
      candidate.occurredDateManualOverride = false;
      candidate.occurredDateDerivedFrom = "reference_date_partial_year";
      return;
    }
...
function resolvePreviousEventDateRelations(candidates) {
  ...
  candidates.forEach(function (candidate, index) {
    ...
    if (candidate.type === "installment_payment" && validIsoDate(candidate.occurredDate) && candidate.occurredDateDerived !== true) return;
...
function canonicalItemFromWire(planItem) {
  ...
  for (var index = 0; index < planItem.fields.length; index += 1) {
    ...
    seen.add(name);
    if (item.eventType === "remaining_unpaid" && name !== "amountEur") continue;
...
function materializeAuthoritativeLunaPlan(review, context, sourceText) {
  ...
  items.forEach(function (item) {
    ...
    for (var repeatIndex = 0; repeatIndex < count; repeatIndex += 1) {
      ...
      var partialCalendarConstraint = Boolean(item.occurredDateRelation
      ...
      if (item.occurredDateRelation && !partialCalendarConstraint) {
      ...
      events.push(normalizeEvent({
        ...
        occurredDatePrecision: item.occurredDatePrecision,
        dateRelation: item.occurredDateRelation && (partialCalendarConstraint || !occurredDate)
  ...
  events = bindDateRelationAnchors(events);
  events = resolveReferenceDateRelations(events, context && context.referenceDate);
  events = resolvePreviousEventDateRelations(events);
...
function validateLeanAmountEvidence(items, context, sourceText) {
  ...
  for (var index = 0; index < list.length; index += 1) {
    ...
    if (!amount) reason = "luna_amount_invalid";
      ...
      if (noAmountNumber && item.eventType === "remaining_unpaid" && balance > 0) {
        item.amountEur = balance;
        amount = balance;
      }
      if (noAmountNumber && item.eventType === "paid_in_full" && balance > 0) {
        item.amountEur = balance;
        amount = balance;
      }
      ...
      if (item.eventType === "remaining_unpaid" && directlySupported && Math.abs(amount - balance) >= 0.005) reason = "luna_explicit_remaining_amount_conflict";
      else if (!directlySupported && !contextDerived) reason = "luna_amount_evidence_unsupported";
    }
    if (reason) {
...
function materializeLunaFieldPlan(review, context, sourceText, contract) {
  ...
  lunaPolicy.assertAdapterOperations(["schema_validation", "catalog_id_mapping", "deterministic_arithmetic", "ledger_application", "human_review_projection"]);
  var sourceDocument = structuredContracts.createDocument(sourceText);
  var fullSpan = { start: 0, end: String(sourceText || "").length, text: String(sourceText || "") };
  ...
  var events = items.map(function (item, index) {
    ...
    var canonicalEvidenceSpan = structuredContracts.createEvidenceSpan(sourceDocument, eventSpan.text, "clause-luna-" + (index + 1), eventSpan.start);
    var occurredDateApproximate = item.occurredDateApproximate === true || item.occurredDateStatus === "approximate";
    var occurredDateUnknown = item.occurredDateUnknown === true || item.occurredDateStatus === "unknown";
    var occurredDate = item.occurredDate == null ? null : trimText(item.occurredDate, 120) || null;
    var supportedDateRelation = item.occurredDateRelation && canonicalRelationNumbersSupported(item.occurredDateRelation, item.occurredDateEvidenceText);
    var dateRelation = supportedDateRelation && !occurredDateUnknown && !occurredDateApproximate
      ? Object.assign({ field: "occurredDate" }, item.occurredDateRelation)
      : null;
    return {
      candidateId: "candidate-" + (index + 1),
  ...
  finalized.candidates = finalized.candidates.map(function (candidate, index) {
    var ledgerAfter = finalized.ledger && finalized.ledger[index] && positiveAmount(finalized.ledger[index].afterEur);
    return Object.assign({}, candidate, candidate.type === "remaining_unpaid" && ledgerAfter != null ? { amount: ledgerAfter } : {}, {
      cardNumber: index + 1,
      cardTypeId: CARD_ID_BY_TYPE[candidate.type] || CARD_ID_BY_TYPE.custom,
...
var HISTORY_LUNA_INSTRUCTIONS = [
  ...
  "Installment dates: preserve explicit/unknown/approximate dates. A relative phrase with a fixed offset and a stated day such as 'mesec dni nazaj 12.' is exact: output final ISO v plus its relation. A calendar-relative month without a day, such as 'prejšnji mesec', is a partially known exact constraint: return v=null and r=[602,611,621,631,1,643,null], never approximate; the adapter derives the known year-month and the UI asks only for dayOfMonth. Equivalent N-month/year expressions preserve their amount and calendar precision. Mark approximate only when the source explicitly says približno/nekje/okoli; a dropped-letter typo never makes an otherwise clear relation approximate. Explicit cadence is material: every card 2..N MUST include field 2 with previous_event +K relation, even when v is null; omitting that relation is invalid. With explicit first, later cards use previous_event +K. With explicit last, calculate earlier dates backward, but cards 2..N still carry previous_event +K. Only with no endpoint may card 1 omit field 2; then cards 2..N use v=null,r=[601,611,622,633,K,unitId,null]. Cadence alone never invents the first date; referenceDate is only a human-input boundary.",
...
].join(" ") + " " + lunaErrorKnowledge.promptContext("history");
...
function parseLeanCompactPlan(output, sourceText) {
  ...
  var contractProposal = hasResponseKind ? parsed : Object.assign({}, parsed, { k: parsed.p.length ? null : 1 });
  var parsedContract = RESPONSE_ZOD_SCHEMA.safeParse(contractProposal);
  var usesLegacyFieldRows = parsed.p.some(function (item) { return item && Array.isArray(item.f); });
  if (parsedContract.success) parsed = parsedContract.data;
  if (!parsed.p.length) {
  ...
  if (parsed.q != null || (hasResponseKind && parsed.k != null)) return { ok: false, reason: "luna_compact_plan_with_clarification" };
  ...
  if (!parsedContract.success && !usesLegacyFieldRows) return { ok: false, reason: "luna_compact_schema_invalid" };
...
function clarificationWithEvidence(clarification, sourceText, evidenceText) {
  if (!clarification) return null;
  var document = structuredContracts.createDocument(sourceText);
  var span = structuredContracts.createEvidenceSpan(document, evidenceText, "clause-1");
  if (!span || !structuredContracts.verifyEvidenceSpan(document, span)) return null;
  return Object.assign({}, clarification, { evidenceSpan: span });
}
...
async function analyze(text, context, options) {
  ...
  if (compactPlan.ok && (compactPlan.verdict === "clarification" || compactPlan.verdict === "warning")) {
    if (compactPlan.verdict === "warning") {
      return leanBlockedResult(context, lunaRequestJson, payload, true, "luna_validation_warning", "VALIDATION_WARNING", clarificationWithEvidence({
        question: compactPlan.question, clauseId: "clause-1", round: 0, maxRounds: MAX_CLARIFICATION_ROUNDS, kind: "warning",
      }, sourceInput, compactPlan.evidenceText));
    }
    ...
    return leanBlockedResult(context, lunaRequestJson, payload, true, "luna_clarification_requested", "CLARIFICATION_REQUIRED", clarificationWithEvidence({
...
module.exports = {
  ...
  _test: {
    ...
    validateLeanAmountEvidence: validateLeanAmountEvidence, validateLeanEvidenceCoverage: validateLeanEvidenceCoverage,
    validateLunaDebtInvariant: validateLunaDebtInvariant,
