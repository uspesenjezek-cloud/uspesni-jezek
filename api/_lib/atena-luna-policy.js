...
async function requestOpenAiRaw(options) {
  options = options || {};
...
/*
 * Strukturirane Atenine zahteve gredo skozi AI SDK + Zod. Ne-strukturirane
 * zahteve in neposredni transportni testi ostanejo na preverjenem surovem
 * transportu. Posamezni tok še vedno opravi svoje poslovne/evidence kontrole.
 */
async function requestOpenAi(options) {
  options = options || {};
  var request = null;
  try { request = JSON.parse(options.body); } catch (_error) {}
  var format = request && request.text && request.text.format;
  if (!format || format.type !== "json_schema" || !format.schema || !format.name) return requestOpenAiRaw(options);

  var z = require("zod");
  var aiSdk = require("./atena-ai-sdk");
  var schema;
  try { schema = z.fromJSONSchema(format.schema); }
  catch (_schemaError) {
    var requestError = new Error("Lunina izhodna shema ni veljavna.");
    requestError.code = "LUNA_INVALID_REQUEST";
    requestError.status = 400;
    requestError.retryable = false;
    throw requestError;
  ...
  try {
    var result = await aiSdk.generateStructured({
      ...
      reasoningEffort: request.reasoning && request.reasoning.effort,
      safetyIdentifier: request.safety_identifier,
      promptCacheKey: request.prompt_cache_key,
      promptCacheRetention: request.prompt_cache_options && request.prompt_cache_options.ttl === "24h" ? "24h" : undefined,
      textVerbosity: request.text && request.text.verbosity,
      requestBytes: Buffer.byteLength(options.body, "utf8"),
      legacyRequestBody: options.body,
    ...
    return {
      response: null,
      payload: {
        output_text: JSON.stringify(result.output),
        usage: { input_tokens: result.usage.inputTokens, output_tokens: result.usage.outputTokens, total_tokens: result.usage.totalTokens },
        _atenaAiSdk: { adapter: result.adapterVersion, calls: result.calls },
      },
      attempts: result.attempts,
      elapsedMs: result.elapsedMs,
    };
  } catch (error) {
    if (error && error.code !== "ATENA_AI_SDK_INVALID_OUTPUT") throw error;
    return {
      response: null,
      payload: {
        output_text: typeof error.text === "string" ? error.text : "",
        usage: error.usage || null,
        _atenaAiSdk: { adapter: aiSdk.SDK_ADAPTER_VERSION, calls: Number(error.attempts) || 1, invalidOutput: true },
      },
...
var SAFE_ERROR_CODES = new Set([
  ...
  "LUNA_INVALID_GOAL_PLAN", "LUNA_INVALID_HISTORY_PLAN", "LUNA_INVALID_ADVISOR_PLAN", "LUNA_INVALID_REQUEST", "AI_CAPACITY_UNAVAILABLE",
...
module.exports = Object.freeze({
  ...
  requestOpenAiRaw: requestOpenAiRaw,
