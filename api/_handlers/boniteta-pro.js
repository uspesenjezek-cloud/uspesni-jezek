...
var paidAdmission = require("../_lib/openregister-paid-admission");
...
function profileFromCompletedJob(job) {
  ...
  if (identityStatus === "confirmed_impressum") {
    var dokaziloJeVezano = evidence.status === "validated_reference" && evidence.serverProofVerified === true &&
      evidence.proofStatus === "verified" && Boolean(evidence.evidenceId) &&
      /^[a-f0-9]{64}$/.test(String(evidence.evidenceFingerprint || "")) &&
      /^[a-f0-9]{64}$/.test(String(evidence.screenshotSha256 || "")) &&
      evidence.evidenceJobId === request.evidenceJobId &&
      evidence.evidenceFingerprint === request.evidenceFingerprint &&
      evidence.screenshotSha256 === request.evidenceScreenshotSha256 &&
      identityEvidenceContract.kanonizirajSpletniUrl(evidence.canonicalInputUrl) === identityEvidenceContract.kanonizirajSpletniUrl(request.spletnaStran) &&
      identityEvidenceContract.kanonizirajSpletniUrl(evidence.finalLegalUrl) === identityEvidenceContract.kanonizirajSpletniUrl(request.evidenceFinalLegalUrl);
    if (!dokaziloJeVezano) throw Object.assign(new Error("Impressum potrditev nima istega strežniško preverjenega dokaznega posnetka."), {
      status: 409,
  ...
  return {
    ...
    companyStatus: companyStatusSafety.safeCompanyStatus(identity.active === true ? "active" : identity.active === false ? "inactive" : "unknown", result, companyId),
...
function registryProfile(company) {
  ...
  var safeActive = companyStatusSafety.safeActive(company.active, company, company.company_id);
  return {
    ...
    latestCheck: { source: "openregister_verified_search", identityStatus: "verified_register", identity: { status: "verified_register", companyId: company.company_id, naziv: company.name, registerNumber: company.register_number, registerCourt: company.register_court, active: safeActive, naslov: address.street || "", postnaStevilka: address.postal_code || "", kraj: address.city || "" }, result: { level: "yellow", title: "Profil ustvarjen iz preverjenega registrskega zadetka" } },
  };
}
...
async function paidOpenRegister(req, res, auth, body, options, provider) {
  var outcome = await paidAdmission.execute({
    cfg: authoritativeConfig(),
    userId: auth.user.id,
    idempotencyKey: req.method === "GET" ? query(req, "idempotencyKey") : body.idempotencyKey,
    action: options.action,
    profileId: options.profileId || null,
    binding: options.binding,
    credits: options.credits,
  }, provider);
  return json(res, outcome.statusCode, outcome.payload);
}
...
async function pro(req, res, cfg, auth) {
  ...
  if (action === "company_lookup") {
    ...
    return paidOpenRegister(req, res, auth, body, {
      action: action,
      binding: "company:" + companyId.toUpperCase(),
      credits: 10,
    }, async function () {
      return { ok: true, company: await openregister.companyDetails(companyId), creditsUsed: 10 };
    });
  }
  if (action === "search") {
  ...
  if (action === "document") {
    ...
    return paidOpenRegister(req, res, auth, body, {
      ...
      binding: "document:" + boundDocumentId,
      credits: 10,
    }, async function () {
      return { ok: true, document: await openregister.document(boundDocumentId, false), creditsUsed: 10 };
    });
  }
  if (action === "document_realtime") {
    ...
    return paidOpenRegister(req, res, auth, body, {
      action: action,
      profileId: profile.id,
      binding: "realtime-document:" + String(profile.company_id).toUpperCase() + ":" + documentCategory,
      credits: 10,
    }, async function () {
      return { ok: true, document: await openregister.realtimeDocument(profile.company_id, documentCategory), creditsUsed: 10 };
    });
  }
  if (action === "transparency_order") {
    ...
    return paidOpenRegister(req, res, auth, body, {
      action: action,
      profileId: profile.id,
      binding: "transparency:" + transparencyCompanyId.toUpperCase(),
      credits: 25,
    }, async function () {
      ...
      return { ok: true, extract: orderedExtract, extractProof: resourceProof.sign(auth.user.id, profile.id, "transparency_extract", orderedId), creditsUsed: 25 };
    });
  }
  ...
  if (action === "section") {
    ...
    var sectionCredits = sectionConfig.credits + (normalizedRealtime ? 10 : 0);
    return paidOpenRegister(req, res, auth, body, {
      action: action,
      profileId: profile.id,
      binding: "section:" + String(profile.company_id).toUpperCase() + ":" + section + ":" + (normalizedRealtime ? "realtime" : "standard"),
      ...
      return { ok: true, section: section, data: saved.payload, cache: { hit: false, fetchedAt: saved.fetched_at, expiresAt: saved.expires_at, creditsUsed: saved.credits_used, sourceMode: saved.source_mode } };
    });
  }
...
async function handler(req, res) {
  ...
  catch (err) { console.error("[boniteta-pro]", err.code || err.message, err.details || ""); var failure = { ok: false, code: err.code || "BONITETA_PRO_FAILED", retryable: err.retryable === true, napaka: err.status ? err.message : "Operacija ni uspela." }; if (Number.isFinite(Number(err.retryAfterMs)) && Number(err.retryAfterMs) > 0) failure.retryAfterMs = Number(err.retryAfterMs); return json(res, err.status || 502, failure); }
}
