...
async function recordForUser(cfg, userId, archiveId) {
  const rows = await supabase.pridobiVrstice(cfg, "pos_archive_records",
    ...
    "&select=id,user_id,invoice_id,work_order_id,procedure_version_id,source_table,source_id,document_kind,original_media_type,sha256,byte_size,storage_bucket,storage_path,archived_at,retention_not_before&limit=1");
  ...
  return {
    ...
    records: records.map(function (record) {
      ...
      return {
        ...
        procedureVersionId: record.procedure_version_id,
