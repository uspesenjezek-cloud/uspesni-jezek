...
async function handler(req, res) {
  ...
  catch (error) { console.error("[pos-pogodba-pdf:config]", error && error.stack || error); return json(res, 500, { ok: false, code: "SERVER_NOT_CONFIGURED", napaka: "Strežnik trenutno ni pravilno nastavljen." }); }
  ...
  try {
    ...
    json(res, Number(error && error.status || 500), { ok: false, code: error && error.code, retryable: error && error.retryable === true, retryAfterMs: error && error.retryAfterMs, napaka: "PDF pogodbenega potrdila ni bil ustvarjen." });
  }
}
