-- Različni načini zaključka računa uporabljajo isti atomski ukrep.
-- Denarno plačilo se v zadeva_placila zapiše samo pri dejanskem plačilu;
-- kompenzacija, dobropis in storno zaključijo dolg brez lažnega denarnega priliva.
...
begin
  ...
  if p_action_type = 'partial_payment' then
    ...
    set opomin_nacrt = p_new_plan,
        preostali_dolg = 0,
        placano_skupaj = case when v_settlement_type = 'full' then placano_skupaj + preostali_dolg else placano_skupaj end,
        znesek = 0,
        status = 'Rešeno',
