...
create policy "Uporabnik vidi svoje nastavitve kartic" on public.opomin_kartice_nastavitve for select to authenticated using ((select auth.uid()) = user_id);
create policy "Uporabnik ustvari svoje nastavitve kartic" on public.opomin_kartice_nastavitve for insert to authenticated with check ((select auth.uid()) = user_id);
create policy "Uporabnik posodobi svoje nastavitve kartic" on public.opomin_kartice_nastavitve for update to authenticated using ((select auth.uid()) = user_id) with check ((select auth.uid()) = user_id);
