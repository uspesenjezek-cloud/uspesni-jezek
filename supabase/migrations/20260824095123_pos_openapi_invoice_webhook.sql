-- Authenticated Openapi callbacks update the existing immutable delivery trail.

create or replace function private._pos_apply_openapi_invoice_event(
  p_provider_reference text,
  p_state text,
  p_external_status text default '',
...
begin
  ...
  where provider='openapi' and provider_reference=trim(p_provider_reference)
  for update;
  if not found then return null; end if;

  v_status := case when v_state='DONE' then 'delivered' when v_state='ERROR' then 'failed' else 'sent' end;
  v_event := case when v_state='DONE' then 'delivered' when v_state='ERROR' then 'failed' else 'sent' end;

  if v_delivery.status='delivered' and v_status<>'delivered' then return v_delivery; end if;
  ...
  update public.pos_invoice_deliveries set
    status=v_status,
    sent_at=case when v_status in ('sent','delivered') then coalesce(sent_at,v_event_at) else sent_at end,
    delivered_at=case when v_status='delivered' then coalesce(delivered_at,v_event_at) else delivered_at end,
    completed_at=case when v_status in ('delivered','failed') then coalesce(completed_at,v_event_at) else completed_at end,
...
create or replace function public.pos_apply_openapi_invoice_event(
  p_provider_reference text,
  p_state text,
  p_external_status text default '',
  p_event_at timestamptz default null
...
as $$
  select private._pos_apply_openapi_invoice_event(
    p_provider_reference,p_state,p_external_status,p_event_at
  );
$$;

revoke all on function private._pos_apply_openapi_invoice_event(text,text,text,timestamptz) from public,anon,authenticated;
revoke all on function public.pos_apply_openapi_invoice_event(text,text,text,timestamptz) from public,anon,authenticated;
grant execute on function private._pos_apply_openapi_invoice_event(text,text,text,timestamptz) to service_role;
grant execute on function public.pos_apply_openapi_invoice_event(text,text,text,timestamptz) to service_role;
