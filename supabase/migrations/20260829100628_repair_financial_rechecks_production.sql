create table if not exists public.boniteta_ponovne_preverbe (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
  profile_id uuid not null references public.boniteta_profili(id) on delete cascade,
  reason text not null constraint boniteta_ponovne_preverbe_reason_check check (reason in ('financial_caution')),
  interval_days integer not null constraint boniteta_ponovne_preverbe_interval_days_check check (interval_days between 1 and 365),
...
do $$
begin
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'boniteta_ponovne_preverbe' and policyname = 'Uporabnik bere svoje ponovne preverbe') then
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'boniteta_ponovne_preverbe' and policyname = 'Uporabnik ustvari svoje ponovne preverbe') then
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'boniteta_ponovne_preverbe' and policyname = 'Uporabnik posodobi svoje ponovne preverbe') then
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'boniteta_ponovne_preverbe' and policyname = 'Uporabnik izbriše svoje ponovne preverbe') then
...
create index if not exists boniteta_ponovne_preverbe_due_idx on public.boniteta_ponovne_preverbe(scheduled_for) where status = 'scheduled';
...
alter table public.mehka_boniteta_opravila add column if not exists financial_recheck_id uuid references public.boniteta_ponovne_preverbe(id) on delete set null;
create index if not exists mehka_boniteta_financial_recheck_idx on public.mehka_boniteta_opravila(financial_recheck_id) where financial_recheck_id is not null;
