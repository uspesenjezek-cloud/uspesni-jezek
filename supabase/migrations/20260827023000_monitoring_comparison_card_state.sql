-- Po vsaki samodejni preverbi ohranimo prejšnji in novi posnetek ter stanje
-- kartice. Tako naslednja preverba primerja isto podjetje z zadnjim rezultatom.
...
begin
  ...
  update public.boniteta_projektna_spremljanja
     set last_check_at = now(),
         ...
         request_payload = case when p_success then
           jsonb_set(
             jsonb_set(request_payload, '{monitoringBaseline}', coalesce(p_result->'monitoringBaseline', '{}'::jsonb), true),
             '{monitoringCardState}', coalesce(p_result->'monitoringCardState', '{}'::jsonb), true
           )
  ...
  update public.boniteta_profili
     set latest_check = case when p_success then coalesce(p_result, '{}'::jsonb) - 'monitoringBaseline' else latest_check end,
         checked_at = case when p_success then now() else checked_at end,
         updated_at = now()
   where id = v_monitor.profile_id;

  if p_success and v_status in ('found', 'possible_match', 'match', 'warning') then
