...
assert.equal(engine.fields.length, 60);
...
assert.deepEqual(Object.fromEntries(engine.areas.map((area) => [area.code, area.moduleIds.map((id) => engine.modules.find((module) => module.id === id).question)])), {
  ...
  pogodba:[
    "Kakšna je osnovna oblika sodelovanja in kateri dodatni režimi veljajo?",
  ...
  tveganja:[
    "Kakšna je vloga osebe ali podjetja, ki vam je poslalo ponudbo?",
    "Kdo je pravni ponudnik in katera veljavna dovoljenja ali dokazila ima?",
    "Ali bo ponudnik vključil podizvajalce in od katerih predpogojev je izvedba odvisna?",
...
assert.equal(engine.modules.find((module) => module.id === 4009).kombinacijaId, "pogodbe-stej-ponovi-sestej", "P01 mora deklarirati prvo produkcijsko reaktivno kombinacijo");
assert.equal(engine.sestavi({ moduleIds:[4009] }).modules[0].kombinacijaId, "pogodbe-stej-ponovi-sestej", "materializirani modul mora ohraniti reaktivno vezavo");
