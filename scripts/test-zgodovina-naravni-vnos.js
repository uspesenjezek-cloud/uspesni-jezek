...
async function main() {
  ...
  assert.ok(typoCanonical.semanticPlan.requestBytes > 10000 && typoCanonical.semanticPlan.requestBytes < 26000);
  ...
  var relationMaterialized = parser._test.materializeLunaFieldPlan({ items: [
    { eventType: "partial_payment", amountEur: 364, evidenceText: "pred 6 dnevi 364", occurredDate: "2026-08-01", occurredDateRelation: { anchor: "reference_date", direction: -1, amount: 6, unit: "day" }, providedFieldIds: [1, 2] },
    { eventType: "partial_payment", amountEur: 212, evidenceText: "dva dni pozneje 212", occurredDate: "2026-08-08", occurredDateRelation: { anchor: "previous_event", direction: 1, amount: 2, unit: "day" }, providedFieldIds: [1, 2] },
  ] }, { referenceDate: "2026-08-29", originalDebt: 9446, remainingDebt: 9446 }, "pred 6 dnevi 364 dva dni pozneje 212");
  assert.deepEqual(relationMaterialized.result.candidates.map(function (candidate) { return candidate.occurredDate; }), ["2026-08-23", "2026-08-25"], "eksplicitne Lunine relacije morajo deterministično preglasiti neskladen ISO prikaz");
  var unsupportedRelation = parser._test.materializeLunaFieldPlan({ items: [
    { eventType: "partial_payment", amountEur: 431, evidenceText: "pred enim tednom 431", occurredDate: "2026-08-22", occurredDateEvidenceText: "pred enim tednom", occurredDateRelation: { anchor: "reference_date", direction: -1, amount: 7, unit: "week" }, providedFieldIds: [1, 2] },
  ] }, { referenceDate: "2026-08-29", originalDebt: 9446, remainingDebt: 9446 }, "pred enim tednom 431");
  assert.equal(unsupportedRelation.result.candidates[0].occurredDate, "2026-08-22", "relacija 7 tednov ne sme preglasiti ISO, ko evidence izrecno podpira število 1");
  var incompatibleRemainingField = parser._test.parseLeanCompactPlan(JSON.stringify({
    p: [{ c: 5, e: "preostali dolg ni placan", i: [1, 2], v: [900, "2026-08-29"], x: ["preostali dolg", "preostali dolg ni placan"], r: [[], [601, 611, 621, 632, 0, 641, null]] }], q: null, x: null, k: null,
  }), "preostali dolg ni placan");
  assert.equal(incompatibleRemainingField.ok, true);
  assert.equal(incompatibleRemainingField.items[0].occurredDate, null, "polje zunaj zaprtega kataloga kartice se ne sme materializirati");
  var strayTopEvidence = parser._test.parseLeanCompactPlan(JSON.stringify({
    p: [{ c: 1, e: "danes 100", i: [1], v: [100], x: ["100"], r: [[]] }],
    q: null, x: "danes 100", k: null,
  }), "danes 100");
  assert.equal(strayTopEvidence.ok, true, "osamljeni clarification evidence brez vprašanja ne sme podreti sicer veljavnega plana");

  var arithmeticItems = [
    { eventType: "partial_payment", amountEur: 1011, evidenceText: "mesec dni nazaj 1011", amountEvidenceText: "1011" },
    { eventType: "partial_payment", amountEur: 583, evidenceText: "dva tedna nazaj 583", amountEvidenceText: "583" },
    { eventType: "partial_payment", amountEur: 353, evidenceText: "danes 353 eur", amountEvidenceText: "353 eur" },
    { eventType: "remaining_unpaid", amountEur: 748, evidenceText: "drugo pa ni placal", amountEvidenceText: "drugo pa ni placal" },
  ];
  var arithmeticSource = "mesec dni nazaj 1011 dva tedna nazaj 583 danes 353 eur drugo pa ni placal";
  var arithmeticValidation = parser._test.validateLeanAmountEvidence(arithmeticItems, { remainingDebt: 9446, originalDebt: 9446 }, arithmeticSource);
  assert.equal(arithmeticValidation.ok, true);
  assert.equal(arithmeticItems[3].amountEur, 7499, "neizrecni preostanek mora vedno slediti determinističnemu ledgerju");
  var explicitConflictItems = [
    { eventType: "partial_payment", amountEur: 100, evidenceText: "placal 100", amountEvidenceText: "100" },
    { eventType: "remaining_unpaid", amountEur: 500, evidenceText: "ostalo 500 ni placal", amountEvidenceText: "500" },
  ];
  var explicitConflict = parser._test.validateLeanAmountEvidence(explicitConflictItems, { remainingDebt: 1000, originalDebt: 1000 }, "placal 100 ostalo 500 ni placal");
  assert.equal(explicitConflict.ok, false);
  assert.equal(explicitConflict.reason, "luna_explicit_remaining_amount_conflict", "izrecnega protislovnega preostanka adapter ne sme tiho popraviti");
  ...
  assert.deepEqual([
    ...
    monthOnlyCanonical.candidates[0].occurredDateKnownYearMonth,
  ], [null, false, null, "2026-07"], "splošen mesec mora ohraniti znani leto in mesec brez izmišljenega dneva");
  assert.deepEqual(monthOnlyCanonical.questionPlan[0].missing, ["occurredDate"], "znani mesec mora zahtevati samo manjkajoči dan");
  ...
  assert.deepEqual(incompleteMiddleCanonical.candidates.map(function (candidate) { return candidate.occurredDate || candidate.occurredDateKnownYearMonth; }), ["2026-05", "2026-07", "2026-08-28"]);
  assert.deepEqual(incompleteMiddleCanonical.questionPlan.map(function (question) { return question.missing; }), [["occurredDate"], ["amount", "occurredDate"], []]);
  ...
  assert.match(html, /neplacila-zgodovina\.css\?v=20260902-atena-sklop-visje-v18/);
  assert.match(embeddedHtml, /neplacila-zgodovina\.css\?v=20260902-atena-sklop-visje-v18/);
  assert.match(historyCss, /\.zgodovina-nacina\s*\{[^}]*margin:\s*-8px 0 10px;/s, "celoten Atenin sklop mora biti tesno ob naslovnem besedilu");
  assert.match(historyCss, /\.zgodovina-nacina \.zgodovina-nacina__izbira\s*\{[^}]*width:\s*100%;[^}]*box-sizing:\s*border-box;[^}]*transform:\s*translateY\(4px\);/s, "pill mora v obeh načinih ohraniti isto širino in položaj");
  assert.match(historyCss, /\.zgodovina-nacina:has\(\.zgodovina-ai__vnos\) \.zgodovina-nacina__izbira\s*\{[^}]*margin:\s*0 0 -12px;/s, "pill mora v naravnem načinu prekrivati vnos");
  assert.match(historyCss, /\.zgodovina-nacina:not\(:has\(\.zgodovina-ai__vnos\)\) \.zgodovina-nacina__izbira\s*\{[^}]*margin-bottom:\s*8px;/s, "ročni način mora ohraniti isti pill brez prekrivanja ročnih kartic");
  assert.match(historyCss, /\.zgodovina-nacina:has\(\.zgodovina-ai__vnos\) \.zgodovina-ai__vnos textarea\s*\{[^}]*padding-top:\s*26px;[^}]*border-radius:\s*25px;/s, "vnos mora imeti enako mehko krivuljo tudi na spodnjih vogalih");
  assert.match(historyCss, /\.zgodovina-ai__vnos textarea\s*\{[^}]*min-height:\s*136px;/s, "chat okno mora imeti povečano višino");
  ...
  assert.match(historyCss, /\.stran--neplacila-zgodovina \.izvedba-action-sheet__panel\s*\{[^}]*box-shadow:\s*0 12px 26px rgba\(73, 61, 45, 0\.14\),\s*0 2px 5px rgba\(73, 61, 45, 0\.06\);/s, "Atenin vnos dogodkov mora imeti enak lebdeči učinek kot priporočilna kartica");
  assert.match(historyCss, /\.atena \.atena__povrsina\s*\{[^}]*box-shadow:\s*0 12px 26px rgba\(73, 61, 45, 0\.14\),\s*0 2px 5px rgba\(73, 61, 45, 0\.06\);/s, "poznejši slog Atenine površine ne sme prepisati lebdečega učinka");
  assert.match(historyCss, /\.atena \.atena__panel\.atena__povrsina\s*\{[^}]*width:\s*calc\(100% - 12px\);[^}]*max-width:\s*calc\(var\(--sirina-vsebine\) - 12px\);[^}]*margin-inline:\s*auto;/s, "lebdeča Atenina kartica mora biti rahlo ožja in sredinsko poravnana");
