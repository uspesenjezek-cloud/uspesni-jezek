...
assert.match(uiSource, /Priporočamo: Mešane ponudbe<\/strong>/);
assert.match(uiSource, /class="lp-filter-ponudb__priporocilo" role="note"/);
assert.match(cssSource, /\.lp-filter-ponudb__priporocilo\s*\{[\s\S]*?grid-template-columns:\s*32px minmax\(0, 1fr\);[\s\S]*?border-radius:\s*12px;/);
