...
const widgets = templates.categories["3.0"];
...
const widgetGallery = templates.renderGallery("3.0");

assert.equal(templates.version, "atena-card-templates-v3");
assert.equal(templates.categories["1.0"].records.length, 17, "Skupni vir mora vsebovati vseh 17 kartic zgodovine neplačnika.");
assert.ok(templates.categories["1.0"].records.every((record) => templates.templates.some((template) => template.id === record.templateId)), "Vsaka zgodovinska kartica mora kazati na kanonično Nazorjevo zasnovo.");
const APPROVED_COUNT = 62;
const DRAFT_COUNT = templates.templates.length - APPROVED_COUNT;
const TOTAL_COUNT = templates.templates.length;
assert.equal(templates.approvedTemplateIds.length, APPROVED_COUNT, "Vseh 62 zasnov iz NAZORJEVA mora biti v zaklenjenem kanoničnem naboru.");
assert.ok(templates.templates.slice(0, 30).every((template) => template.approved && template.approvedVersion === "2026-08-30"));
assert.ok(templates.templates.slice(30, APPROVED_COUNT).every((template) => template.approved && template.approvedVersion === "2026-08-31"));
assert.ok(templates.templates.slice(APPROVED_COUNT).every((template) => template.approved === false && template.approvedVersion === null), "Vsak widget nad 62 mora biti neodobren osnutek iz NAZORJEVA-TEST.");
assert.equal(new Set(templates.templates.map((template) => template.id)).size, TOTAL_COUNT, "ID-ji zasnov morajo biti enolični.");
assert.ok(templates.templates.every((template) => template.title && template.question && template.coverage && template.rgb));
assert.deepEqual(templates.templates.map((template) => template.number), Array.from({ length:TOTAL_COUNT }, (_, index) => index + 1));
assert.equal((gallery.match(/data-template-id=/g) || []).length, TOTAL_COUNT);
assert.equal((gallery.match(/data-template-card=/g) || []).length, TOTAL_COUNT);
const noSaveCount = templates.templates.filter((template) => template.noSave).length;
assert.equal((gallery.match(/Shrani podatke/g) || []).length, TOTAL_COUNT - noSaveCount, "Widgeti z noSave (samo prikaz, brez zbiranja podatka) ne smejo imeti gumba 'Shrani podatke'.");
assert.equal((gallery.match(/Nov osnutek/g) || []).length, DRAFT_COUNT);
...
assert.equal(widgets.version, "atena-card-category-3.0-v1");
assert.equal(widgets.category, "3.0");
assert.equal(widgets.widgets.length, TOTAL_COUNT);
assert.equal(widgets.records.length, TOTAL_COUNT);
assert.deepEqual(widgets.records.map((record) => record.id), templates.templates.map((template) => template.id), "Kategorija 3.0 mora neposredno uporabljati vse kanonične in testne widgete iz istega rendererja.");
assert.equal((widgetGallery.match(/data-card-category="3\.0"/g) || []).length, TOTAL_COUNT);
assert.match(widgetGallery, />3\.0\.01</);
assert.match(widgetGallery, />3\.0\.61</);
assert.match(widgetGallery, />3\.0\.62</);
if (DRAFT_COUNT > 0) assert.match(widgetGallery, />3\.0\.63</, "Osnutek 63 mora biti viden v galeriji kategorije 3.0.");
...
assert.equal((html.match(/data-card-category="3\.0"/g) || []).length, TOTAL_COUNT);
assert.match(html, /Kategorija 2\.0 — ciljni FATHER[\s\S]*Kategorija 3\.0 — Atenini widgeti/, "Kategorija 3.0 mora slediti kategoriji 2.0.");
assert.match(html, new RegExp('data-widget-gallery[^>]*aria-label="' + TOTAL_COUNT + ' Ateninih widgetov kategorije 3\\.0"'));
assert.match(markdown, /Kategorija 3\.0 — Atenini widgeti/);
assert.match(html, /\[data-father-card-id\] \.uj-answer-card__body\s*\{[^}]*grid-template-columns:\s*repeat\(2, minmax\(0, 1fr\)\)/, "Kartice 2.0 morajo vidno uporabljati dve kompaktni koloni.");
assert.match(html, /\[data-father-card-id\] \.uj-card-date\s*\{[^}]*grid-template-columns:\s*minmax\(0, 1fr\)/, "Datum v polovici kartice 2.0 mora zapolniti celoten stolpec.");
assert.match(html, /uj-card-field uj-card-field--wide/, "Daljša besedilna polja kartic 2.0 morajo biti izrecno označena za polno širino.");
assert.equal((html.match(/uj-card-money uj-card-money--unit-only/g) || []).length, 9, "Vseh devet denarnih polj kategorije 2.0 mora uporabljati pravilno poravnavo brez leve ikone.");
assert.match(html, /\.uj-card-money--unit-only input\[type="text"\]\s*\{[^}]*grid-column:\s*2/, "Znesek brez leve ikone mora ostati v širokem srednjem stolpcu.");
assert.match(html, /\.uj-card-money--unit-only b\s*\{[^}]*grid-column:\s*3/, "Znak € mora ostati v ločenem desnem stolpcu.");
...
assert.match(html, /\.uj-card-selection-note\s*\{[^}]*font-size:\s*13px[^}]*white-space:\s*nowrap/, "Potrditev izbire mora ostati v eni vrstici, pisava pa ne sme pod 13px.");
...
[
  ...
  "gradnik-pravila-eskalacije", "sled-izvora-podatka", "prag-rentabilnosti", "drevo-pricakovane-vrednosti", "kaskada-krsitve", "graficni-cenovni-most", "sprememba-in-potrditev"
...
const changeConfirmationCard = gallery.match(/<article[^>]*data-template-card="sprememba-in-potrditev"[\s\S]*?<\/article>/)[0];
...
assert.match(changeConfirmationCard, /Vrsta spremembe[\s\S]*Dodano[\s\S]*Odstranjeno[\s\S]*Vpliv spremembe[\s\S]*Obseg dela[\s\S]*data-change-amount[\s\S]*data-change-days[\s\S]*Potrditev[\s\S]*Ni potrjeno[\s\S]*V preverjanju[\s\S]*Potrjeno/, "Kartica 62 mora vsebovati vse tri interaktivne korake potrjenega modela.");
assert.match(changeConfirmationCard, /data-change-confirm-summary[\s\S]*Dodano · \+680 € · \+2 dni[\s\S]*Sprememba še ni potrjena\./, "Kartica 62 mora jasno povzeti začetno izbiro.");
assert.match(html, /updateChangeConfirmation[\s\S]*data-change-kind[\s\S]*data-change-status[\s\S]*data-change-amount/, "Vrsta, status in številčni vplivi morajo sproti posodabljati povzetek.");
...
assert.match(html, /atena-card-templates-v3/);
assert.match(html, /grid-template-columns:\s*repeat\(3/);
assert.match(html, /min-height:\s*44px/);
assert.match(html, /\[data-combo-options\]\[hidden\]\s*\{\s*display:\s*none;/);
assert.match(markdown, new RegExp(TOTAL_COUNT + ' zasnov kartic'));
if (DRAFT_COUNT > 0) {
  assert.match(markdown, new RegExp('Kartice ' + (APPROVED_COUNT + 1) + '–' + TOTAL_COUNT + ' so novi funkcionalni osnutki'), "Osnutki morajo biti v katalogu jasno označeni kot neodobreni.");
} else {
  assert.match(markdown, /Vseh 62 zasnov je potrjenih, zaklenjenih in shranjenih/);
}
assert.equal((markdown.match(/Nov osnutek/g) || []).length, DRAFT_COUNT);
assert.doesNotMatch(gallery, /Dopolnite \d+\/\d+|>Spremeni</);

console.log("Atena card templates: OK (" + APPROVED_COUNT + " potrjenih iz NAZORJEVA, " + DRAFT_COUNT + " v testu)");
