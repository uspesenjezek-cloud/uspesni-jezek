...
test("podrobnosti uporabljajo podpisane povezave in nimajo lažnega PDF-ja", function () {
  ...
  assert.match(src, /koncani-kartica__rezultat[\s\S]*?<p data-koncani-fit data-fit-min="8\.5">/);
  assert.doesNotMatch(src, /scrollWidth > element\.clientWidth \+ 1|scrollHeight > element\.clientHeight \+ 1/);
  ...
  assert.match(css, /@media \(max-width: 400px\)[\s\S]*?\.koncani-kartica__glavno\s*\{[\s\S]*?grid-template-columns:\s*39px minmax\(0, 1fr\) minmax\(88px, auto\)/);
  assert.match(css, /@media \(max-width: 400px\)[\s\S]*?\.koncani-kartica__spodaj\s*\{[\s\S]*?grid-template-columns:\s*minmax\(0, 1\.45fr\) minmax\(108px, \.82fr\)/);
});
