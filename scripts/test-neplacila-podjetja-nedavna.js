...
assert.match(html, /id="podjetja-sheet-seznam-naslov">Nedavno uporabljena podjetja<\/p>[\s\S]*?id="podjetja-sheet-dodaj-v-kategorije"[^>]*>Dodaj v kategorije<\/button>[\s\S]*?id="podjetja-sheet-dodaj-navodilo-vrstica"[^>]*hidden[\s\S]*?id="podjetja-sheet-dodaj-navodilo">Izberite podjetje spodaj in kategorijo zgoraj\.<\/p>[\s\S]*?id="podjetja-sheet-dodaj-potrdi"[^>]*disabled>Dodaj<\/button>/);
...
assert.match(html, /class="podjetja-sheet__kategorije-dok"[\s\S]*?id="podjetja-sheet-kategorija-vse"[\s\S]*?<span>Vse<\/span>[\s\S]*?id="podjetja-sheet-nedavna"[^>]*>Nedavno<\/button>[\s\S]*?id="podjetja-sheet-kategorije-viewport"[\s\S]*?id="podjetja-sheet-kategorije-seznam"[\s\S]*?id="podjetja-sheet-nova-kategorija"[^>]*aria-label="Nova kategorija">\+<\/button>/);
...
assert.match(html, /id="podjetja-sheet-kategorije-uredi"[^>]*hidden>Uredi kategorije<\/button>/);
...
assert.match(clientJs, /kategorijeUrediGumb\.textContent = urejanjeKategorij \? "Končano" : "Uredi kategorije"/);
...
assert.match(css, /\.nedavna-podjetja__vrstica[\s\S]*?width:\s*100%[\s\S]*?height:\s*48px[\s\S]*?border:\s*0[\s\S]*?background-color:\s*#469c98[\s\S]*?background-image:\s*linear-gradient/);
...
assert.match(css, /\.nedavna-podjetja__drsnik[\s\S]*?inset:\s*0 auto 0 0[\s\S]*?width:\s*100%/);
assert.match(css, /\.nedavna-podjetja__drsnik[\s\S]*?border-radius:\s*24px[\s\S]*?scroll-padding-inline-start:\s*8px[\s\S]*?scroll-snap-type:\s*x proximity/);
assert.doesNotMatch(css, /\.nedavna-podjetja__drsnik[^}]*?(?:-webkit-)?mask-image/);
assert.match(css, /\.nedavna-podjetja__trak[\s\S]*?align-items:\s*center[\s\S]*?height:\s*100%[\s\S]*?padding:\s*0 calc\(var\(--nedavna-cta-sirina\) \+ 8px\) 0 8px/);
...
assert.match(css, /\.nedavna-podjetja__vec[\s\S]*?top:\s*0[\s\S]*?right:\s*0[\s\S]*?width:\s*var\(--nedavna-cta-sirina\)[\s\S]*?height:\s*48px/);
assert.match(css, /\.nedavna-podjetja__vec[\s\S]*?display:\s*grid[\s\S]*?place-items:\s*center[\s\S]*?padding:\s*0 4px 0 16px/);
assert.match(css, /\.nedavna-podjetja__vec[\s\S]*?box-sizing:\s*border-box[\s\S]*?border:\s*0[\s\S]*?border-radius:\s*0 24px 24px 0[\s\S]*?background:\s*#469c98/);
...
assert.match(css, /\.nedavna-podjetja__vec::before[\s\S]*?inset:\s*4px 3px 4px 2px[\s\S]*?border:\s*1px solid rgba\(255, 255, 255, 0\.48\)[\s\S]*?border-left:\s*0[\s\S]*?border-radius:\s*0 20px 20px 0[\s\S]*?rgba\(0, 0, 0, 0\.82\) 10%[\s\S]*?#000 18%/);
...
assert.match(css, /\.podjetja-sheet \.ocena-sheet__panel[\s\S]*?height:\s*min\(92dvh, 820px\)/);
...
assert.match(css, /\.podjetja-sheet__kategorije-dok[\s\S]*?height:\s*152px[\s\S]*?background:\s*transparent/);
assert.doesNotMatch(css, /\.podjetja-sheet__kategorije-dok::before/);
assert.match(css, /\.podjetja-sheet__kategorije-viewport[\s\S]*?inset:\s*56px 0 0[\s\S]*?padding:\s*8px 0[\s\S]*?overflow-x:\s*auto[\s\S]*?scroll-snap-type:\s*x mandatory/);
...
assert.match(css, /\.podjetja-sheet__kategorije-stran[\s\S]*?padding:\s*0 8px[\s\S]*?grid-template-columns:\s*repeat\(6, minmax\(0, 1fr\)\)[\s\S]*?grid-template-rows:\s*repeat\(2, 36px\)/);
...
assert.match(css, /\.podjetja-sheet__kategorija-element--duh[\s\S]*?position:\s*fixed !important[\s\S]*?transform:\s*scale\(1\.045\)/);
assert.match(css, /\.podjetja-sheet__kategorija-element--duh\.podjetja-sheet__kategorija-element--spuscena[\s\S]*?transition:\s*left 165ms/);
assert.match(css, /\.podjetja-sheet__kategorija-element:has\(\.podjetja-sheet__kategorija-gumb--aktiven\)[\s\S]*?background:\s*#469c98/);
...
assert.match(css, /\.podjetja-sheet__kategorije-uredi[\s\S]*?border:\s*1px solid[\s\S]*?border-radius:\s*9px[\s\S]*?background:\s*#f6fbfa/);
