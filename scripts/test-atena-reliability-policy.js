...
async function idempotencyAndConcurrency() {
  ...
  var finishCoordinator = policy.createDistributedCoordinator({
    ...
    contractVersion: "goal-fact-v18", fingerprint: "a".repeat(64),
...
async function ledgerAndStaleResponseInvariants() {
  ...
  var clientContext = {
    ...
    clearTimeout: function (timer) { clearedTimers.push(timer); },
    setInterval: function (callback) { intervalCallback = callback; return 88; },
    clearInterval: function (timer) { clearedIntervals.push(timer); },
  };
  ...
  assert.equal(requestGuard.timedOut(), false);
  requestGuard.dispose();
  assert.deepEqual(clearedTimers, [77], "idempotenten abort mora timeout počistiti natanko enkrat");
  var timeoutGuard = clientContext.UJAtenaRequest.create();
  timeoutCallback();
  assert.equal(timeoutGuard.signal.aborted, true);
  assert.equal(timeoutGuard.timedOut(), true);

  var retryGate = clientContext.UJAtenaRequest.createRetryGate({
    now: function () { return fakeNow; },
    setInterval: clientContext.setInterval,
    clearInterval: clientContext.clearInterval,
  });
  var fetchCalls = 0;
  function guardedClick() { if (!retryGate.isBlocked()) fetchCalls += 1; }
  retryGate.block(2100);
  guardedClick();
  assert.equal(fetchCalls, 0, "retryAfter blokada ne sme sama ali ob kliku sprožiti fetcha");
  assert.equal(Math.ceil(retryGate.remainingMs() / 1000), 3);
  fakeNow = 3100;
  intervalCallback();
  assert.equal(Math.ceil(retryGate.remainingMs() / 1000), 0);
  guardedClick();
  assert.equal(fetchCalls, 1, "isti klik se mora po izteku blokade znova sprostiti");
  assert.deepEqual(clearedIntervals, [88]);
  ...
  assert.match(historyUi, /analizaRetryGate\.isBlocked\(\)/);
  assert.match(goalUi, /ciljRetryGate\.isBlocked\(\)/);
  assert.match(historyUi, /analizaRetryGate\.block\(error\.retryAfterMs\)/, "History mora spoštovati strežniški retryAfterMs brez samodejnega retryja");
  assert.match(goalUi, /ciljRetryGate\.block\(error\.retryAfterMs\)/, "Goal mora spoštovati strežniški retryAfterMs brez samodejnega retryja");
  assert.match(historyUi, /Ponovno lahko poskusite čez/);
  assert.match(goalUi, /Ponovno lahko poskusite čez/);

  var resolverSource = historyUi.match(/function kandidatIzKontrolnika\(kontrolnik\) \{[\s\S]*?\n  \}/)[0];
  var resolverContext = { rezultat: null };
  vm.runInNewContext('var naravni = { candidates: [{ candidateId: "candidate-a" }, { candidateId: "candidate-b" }] }; ' + resolverSource + '; rezultat = kandidatIzKontrolnika({ getAttribute: function (ime) { return ime === "data-ai-candidate-id" ? "candidate-b" : "0"; } });', resolverContext);
  assert.equal(resolverContext.rezultat.indeks, 1, "candidateId mora premagati zastarel indeks");
  vm.runInNewContext('var naravni = { candidates: [{ candidateId: "candidate-a" }, { candidateId: "candidate-b" }] }; ' + resolverSource + '; rezultat = kandidatIzKontrolnika({ getAttribute: function (ime) { return ime === "data-ai-candidate-id" ? "missing" : "0"; } });', resolverContext);
  assert.equal(resolverContext.rezultat.indeks, 0, "indeks mora ostati varen kompatibilnostni fallback");
