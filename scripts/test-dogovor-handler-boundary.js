...
global.fetch = async function (_url, options) {
  ...
  function jsonInput(value) {
    if (typeof value === "string") {
      try { var parsed = JSON.parse(value); if (parsed && parsed.sourceText) return parsed; } catch (_error) {}
      return null;
    }
    if (Array.isArray(value)) {
      for (var index = 0; index < value.length; index += 1) { var arrayMatch = jsonInput(value[index]); if (arrayMatch) return arrayMatch; }
      return null;
    }
    if (value && typeof value === "object") {
      var keys = Object.keys(value);
      for (var keyIndex = 0; keyIndex < keys.length; keyIndex += 1) { var objectMatch = jsonInput(value[keys[keyIndex]]); if (objectMatch) return objectMatch; }
    }
    return null;
  }
  var input = jsonInput(request.input);
  assert.ok(input && input.sourceText, "test mora najti nespremenjen Atenin vhod v AI SDK Responses zahtevi");
