...
async function main() {
  ...
  var odgovor;
  try {
    odgovor = await fetch(osnovniUrl + "/__dev-source", { headers: { Accept: "application/json" }, signal: AbortSignal.timeout(5000) });
  } catch (napaka) {
  ...
  if (!vir.workspaceHash || vir.workspaceHash !== pricakovaniHash) {
    throw new Error(["NAPAČNA LOKALNA DELOVNA KOPIJA.", "Pričakovana mapa: " + pricakovanaMapa, "Strežnik kaže: " + (vir.workspaceName || "neznana mapa") + " · " + (vir.branch || "neznana veja") + " · " + (vir.commit || "neznan commit"), "Ustavi strežnik na tem portu ali zaženi npm run dev iz mape, ki jo dejansko popravljaš."].join("\n"));
  }
  process.stdout.write(["Lokalni vir potrjen.", "Mapa: " + pricakovanaMapa, "Veja: " + (vir.branch || "unknown"), "Commit: " + (vir.commit || "unknown"), "Stanje: " + (vir.dirty ? "lokalne spremembe" : "čisto")].join("\n") + "\n");
}

main().catch(function (napaka) { console.error(napaka.message); process.exitCode = 1; });
