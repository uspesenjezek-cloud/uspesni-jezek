...
const preflightMigration = fs.readFileSync(path.join(repoRoot, "supabase", "migrations", "20260823181854_pos_kosit_preflight_evidence.sql"), "utf8");
...
const testXml = generator.buildXRechnung(invoice({
  is_test: true,
  snapshot: {
    seller: Object.assign({}, invoice().snapshot.seller, {
      legalName: "Internes echtes Unternehmen GmbH", businessEmail: "intern@example.de",
      companySeat: "", registerCourt: "", registerNumber: ""
    }),
    draft: invoice().snapshot.draft
  }
})).toString("utf8");
assert.match(testXml, /TEST-Unternehmen GmbH/);
assert.match(testXml, /HRB TEST 00000 B/);
assert.match(testXml, /testrechnung@beispiel\.de/);
assert.doesNotMatch(testXml, /Internes echtes Unternehmen|intern@example\.de/);
assert.strictEqual(generator.GENERATOR_VERSION, "uj-pos-xrechnung-3");
...
const cardInvoice = invoice({
  snapshot: {
    seller: Object.assign({}, invoice().snapshot.seller, {
      legalName: "Muster Handwerk GmbH & Co. KG", legalForm: "GmbH & Co. KG",
      representative: "Mara Muster"
    }),
    draft: Object.assign({}, invoice().snapshot.draft, { payment_method: "card_external" })
  }
});
const cardXml = generator.buildXRechnung(cardInvoice).toString("utf8");
assert.match(cardXml, /<cbc:PaymentMeansCode name="Kartenzahlung">48<\/cbc:PaymentMeansCode>/);
assert.match(cardXml, /<cbc:CompanyLegalForm>GmbH &amp; Co\. KG; Sitz: Berlin; Registergericht: Amtsgericht Charlottenburg<\/cbc:CompanyLegalForm>/);
...
const preflightProfile = {
  legal_name: "Muster Handwerk GmbH", legal_form: "GmbH", representative: "Mara Muster",
  company_seat: "Berlin", register_court: "Amtsgericht Charlottenburg", register_number: "HRB 12345 B",
  street: "Musterstraße 1", postal_code: "10115", city: "Berlin", business_email: "rechnung@muster-handwerk.de",
  business_phone: "+49 30 1234567", tax_status: "regular", tax_number: "12/345/67890", vat_id: "DE123456789",
  account_holder: "Muster Handwerk GmbH", iban: "DE02120300000000202051"
};
const preflightPayload = Object.assign({}, invoice().snapshot.draft, {
  price_mode: "net", due_days: 14, payment_method: "sepa",
  items: [{ description: "Elektroinstallation", quantity_milli: 2000, unit: "Std.", unit_price_cents: 10000, tax_rate_bps: 1900 }]
});
const preflight = generator.preflightInvoice(preflightProfile, preflightPayload, "11111111-1111-4111-8111-111111111111");
assert.deepStrictEqual({ net: preflight.net_cents, tax: preflight.tax_cents, gross: preflight.gross_cents, due: preflight.due_date }, { net: 20000, tax: 3800, gross: 23800, due: "2026-09-02" });
assert.match(generator.buildXRechnung(preflight).toString("utf8"), /PREFLIGHT-11111111-111/);
const grossPreflight = generator.preflightInvoice(preflightProfile, Object.assign({}, preflightPayload, {
  price_mode: "gross", items: [{ description: "Material", quantity_milli: 1000, unit: "Stk.", unit_price_cents: 119, tax_rate_bps: 1900 }]
}), "22222222-2222-4222-8222-222222222222");
assert.deepStrictEqual({ net: grossPreflight.net_cents, tax: grossPreflight.tax_cents, gross: grossPreflight.gross_cents }, { net: 100, tax: 19, gross: 119 });

function adjustment(type, overrides) {
  const original = invoice();
  const financial = type === "cancellation" || type === "credit_note";
  return Object.assign({
    id: "22222222-2222-4222-8222-222222222222",
    adjustment_number: type === "cancellation" ? "ST-2026-0043" : type === "credit_note" ? "GS-2026-0043" : "KORR-2026-0043",
    adjustment_type: type,
    reason: type === "cancellation" ? "Auftrag vollständig aufgehoben" : type === "credit_note" ? "Preisnachlass" : "Empfängeranschrift berichtigt",
    issued_at: "2026-08-22T00:30:00+02:00",
    delta_net_cents: financial ? -20000 : 0,
    delta_tax_cents: financial ? -3800 : 0,
    delta_gross_cents: financial ? -23800 : 0,
    snapshot: {
...
const creditNoteXml = adjustmentGenerator.buildAdjustmentXRechnung(adjustment("credit_note", {
  delta_net_cents: -5000,
  delta_tax_cents: -950,
  delta_gross_cents: -5950,
  snapshot: Object.assign({}, adjustment("credit_note").snapshot, {
    credit_lines: [{ description: "Kulanznachlass", tax_rate_bps: 1900, net_cents: 5000, tax_cents: 950, gross_cents: 5950 }]
  })
})).toString("utf8");
assert.match(creditNoteXml, /<ubl:CreditNote[^>]+CreditNote-2/);
assert.match(creditNoteXml, /<cbc:ID>GS-2026-0043<\/cbc:ID>/);
assert.match(creditNoteXml, /<cbc:CreditNoteTypeCode>381<\/cbc:CreditNoteTypeCode>/);
assert.match(creditNoteXml, /<cbc:Note>Gutschrift: Preisnachlass<\/cbc:Note>/);
assert.match(creditNoteXml, /<cbc:CreditedQuantity unitCode="C62">1<\/cbc:CreditedQuantity>/);
assert.match(creditNoteXml, /<cbc:LineExtensionAmount currencyID="EUR">50\.00<\/cbc:LineExtensionAmount>/);
assert.match(creditNoteXml, /<cbc:TaxAmount currencyID="EUR">9\.50<\/cbc:TaxAmount>/);
assert.match(creditNoteXml, /<cbc:PayableAmount currencyID="EUR">59\.50<\/cbc:PayableAmount>/);
assert.match(creditNoteXml, /<cac:BillingReference><cac:InvoiceDocumentReference><cbc:ID>RE-2026-0042<\/cbc:ID>/);
assert.throws(() => adjustmentGenerator.buildAdjustmentXRechnung(adjustment("credit_note", {
  snapshot: Object.assign({}, adjustment("credit_note").snapshot, { credit_lines: [] })
})), /davčnih postavk/);
...
assert.match(api, /mode === "preflight"/);
assert.match(api, /readDraft\(cfg, auth\.user\.id, draftId\)/);
assert.match(api, /buildXRechnung\(preflightInvoice\(profile, draft\.payload, draft\.id\)\)/);
assert.match(api, /recordPreflight\(cfg, auth\.user\.id, draft, xml, validation\)/);
...
assert.match(preflightMigration, /create table public\.pos_einvoice_preflight_validations/i);
assert.match(preflightMigration, /alter table public\.pos_einvoice_preflight_validations enable row level security/i);
assert.match(preflightMigration, /revoke all on table public\.pos_einvoice_preflight_validations from public,anon,authenticated/i);
assert.doesNotMatch(preflightMigration, /grant[^;]+pos_einvoice_preflight_validations[^;]+authenticated/i);
assert.match(preflightMigration, /evidence\.payload=p_payload and evidence\.expires_at>now\(\)/i);
assert.strictEqual((preflightMigration.match(/private\.pos_has_einvoice_preflight\(p_draft_id,p_payload\)/g) || []).length, 2);
assert.doesNotMatch(preflightMigration, /p_final_confirmed,\s*true/i);
...
assert.match(terminalHtml, /pos-terminal\.js\?v=20260828-datev-status-v1/);
