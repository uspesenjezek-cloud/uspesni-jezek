...
async function main() {
  ...
  var validPartialDocument = structuredContracts.createDocument(validPartialText);
  assert.ok(validPartial.candidates.every(function (candidate) { return structuredContracts.verifyEvidenceSpan(validPartialDocument, candidate.evidence.span); }), "vsaka history kartica potrebuje preverljiv canonical EvidenceSpan");
  ...
  assert.equal(structuredContracts.verifyEvidenceSpan(structuredContracts.createDocument(overDebtText), blocked.clarification.evidenceSpan), true);
