...
assert.doesNotMatch(historyAdapter, /validateLean|validateLuna|canonicalDateFromRelation|numberEngine|installmentEngine|evidenceIsLinked|semanticRetries|repairReason/);
assert.match(historyAdapter, /canonicalRelationNumbersSupported/,
  "adapter sme preveriti, da Lunina eksplicitna relacija ni v nasprotju z njenim dobesednim dokazom");
assert.match(historyAdapter, /\["schema_validation", "catalog_id_mapping", "deterministic_arithmetic", "ledger_application", "human_review_projection"\]/);
...
assert.equal(goal.CONTRACT_VERSION, "goal-fact-v18");
