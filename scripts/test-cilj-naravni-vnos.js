...
var structuredContracts = require("../api/_lib/atena-structured-contracts");
...
(async function () {
  ...
  assert.equal(engine.CONTRACT_VERSION, "goal-fact-v18");
  ...
  assert.match(body.instructions, /one internal clause-by-clause coverage audit inside this same response/);
  assert.match(body.instructions, /Luna alone decides the semantic number and order of cards/);
  assert.match(body.instructions, /three future installments are three ordered installment_plan cards/);
  assert.match(body.instructions, /derive N and the legal meaning from the whole source, not from a phrase match/);
  assert.match(body.instructions, /one independent future outcome or action per card/);
  assert.match(body.instructions, /Never merge, summarize or hide a requested future action/);
  ...
  assert.equal(structuredContracts.verifyEvidenceSpan(structuredContracts.createDocument("hočem da mi plača v celoti"), exact.goals[0].evidenceSpan), true);
  ...
  var oneLunaInstallment = engine._test.materialize(wirePlan([
    wireGoal(1, "installment_plan", [
  ...
  assert.equal(oneLunaInstallment.length, 1, "Adapter ne sme sam razširiti enega Luninega koraka; Luna je edina avtoriteta za število kartic.");
  ...
  assert.equal(threeInstallments.length, 3, "Lunini trije obročni koraki se morajo preslikati v tri pregledne korake.");
  assert.deepEqual(threeInstallments.map(function (goal) { return goal.stepNumber; }), [1, 2, 3]);
  assert.ok(threeInstallments.every(function (goal) { return goal.goalData.installmentAmount === undefined; }), "Neenakomeren znesek se ne sme tiho zaokrožiti.");
  ...
  assert.equal(fiveInstallments.length, 5, "Pet obrokov se ne sme skrčiti v en korak.");
  ...
  var reportedSource = "hočem da mi plača v treh obrokih če ne grem do odvetnika";
  var reportedInstallments = [1, 2, 3].map(function (number) {
    return wireGoal(number, "installment_plan", [
      wireField("targetAmount", "72", "plača v treh obrokih"),
      wireField("installmentCount", "3", "treh obrokih"),
    ], "plača v treh obrokih");
  });
  var reported = engine._test.materialize(wirePlan(reportedInstallments.concat([
    wireGoal(4, "legal_recovery", [
      wireField("legalOutcome", "legal_route_review", "grem do odvetnika"),
      wireField("legalNote", "če ne grem do odvetnika", "če ne grem do odvetnika"),
    ], "če ne grem do odvetnika"),
  ]), reportedSource), { remainingDebt: 72 }, reportedSource);
  assert.deepEqual(reported.map(function (goal) { return goal.goalId; }), ["installment_plan", "installment_plan", "installment_plan", "legal_recovery"]);
  assert.ok(reported.slice(0, 3).every(function (goal) { return goal.goalData.installmentAmount === "24"; }));
  assert.ok(reported.slice(0, 3).every(function (goal) { return goal.derivedFields[0] === "installmentAmount"; }));
  assert.equal(reported[3].goalData.legalOutcome, "legal_route_review");
  assert.equal(engine._test.equalInstallmentAmount(72, 3), "24");
  assert.equal(engine._test.equalInstallmentAmount(100, 3), null);

  var localeAmountSource = "72,00 evra v treh obrokih";
  var localeAmount = engine._test.materialize(wirePlan([wireGoal(1, "installment_plan", [
    wireField("targetAmount", "72,00", "72,00 evra"),
    wireField("installmentCount", "3", "treh obrokih"),
    { i: engine._test.fieldIdByKey.firstPaymentDate, v: null, o: null, e: "" },
  ], localeAmountSource)], localeAmountSource), { remainingDebt: 72 }, localeAmountSource);
  assert.equal(localeAmount[0].goalData.targetAmount, "72.00");
  assert.equal(localeAmount[0].goalData.installmentAmount, "24");
  assert.ok(localeAmount[0].missing.includes("firstPaymentDate"), "Prazno dovoljeno polje mora postati manjkajoč uporabniški vnos, ne padec načrta.");

  var canonicalEnumSource = "grem do odvetnika";
  var canonicalEnum = engine._test.materialize(wirePlan([wireGoal(1, "legal_recovery", [
    { i: engine._test.fieldIdByKey.legalOutcome, v: "legal_route_review", o: null, e: canonicalEnumSource },
  ], canonicalEnumSource)], canonicalEnumSource), { remainingDebt: 72 }, canonicalEnumSource);
  assert.equal(canonicalEnum[0].goalData.legalOutcome, "legal_route_review", "Zaprt kanonični enum ključ ne sme podreti sicer veljavnega Luninega načrta.");
  var duplicatedCanonicalEnum = engine._test.materialize(wirePlan([wireGoal(1, "legal_recovery", [
    { i: engine._test.fieldIdByKey.legalOutcome, v: "legal_route_review", o: engine._test.valueIdByFieldValue["legalOutcome:legal_route_review"], e: canonicalEnumSource },
  ], canonicalEnumSource)], canonicalEnumSource), { remainingDebt: 72 }, canonicalEnumSource);
  assert.equal(duplicatedCanonicalEnum[0].goalData.legalOutcome, "legal_route_review", "Ujemajoča se ključ in valueId sta tehnično podvojena, ne semantično konfliktna.");

  var numericSerializedEnum = engine._test.materialize(wirePlan([wireGoal(1, "legal_recovery", [
    { i: engine._test.fieldIdByKey.legalOutcome, v: "90102", o: 90102, e: "izvršbo" },
  ], "izvršbo")], "izvršbo"), { remainingDebt: 434 }, "želim izvršbo");
  assert.equal(numericSerializedEnum[0].goalData.legalOutcome, "enforcement", "eksplicitni zaprti optionId mora varno kanonizirati numerično serializacijo");
  var localizedEnum = engine._test.materialize(wirePlan([wireGoal(1, "payment_security", [
    { i: engine._test.fieldIdByKey.securityType, v: "garancija", o: 80101, e: "porostvo" },
  ], "porostvo")], "porostvo"), { remainingDebt: 434 }, "zahtevam porostvo");
  assert.equal(localizedEnum[0].goalData.securityType, "guarantee", "eksplicitni optionId je semantična izbira, lokalizirani v pa ne sme podreti preslikave");
  var wrongFieldOption = engine._test.materialize(wirePlan([wireGoal(1, "payment_security", [
    { i: engine._test.fieldIdByKey.securityType, v: "90102", o: 90102, e: "porostvo" },
  ], "porostvo")], "porostvo"), { remainingDebt: 434 }, "zahtevam porostvo");
  assert.equal(wrongFieldOption, null, "optionId drugega polja mora ostati fail-closed");
  var wrongDisputeDescriptionOption = engine._test.materialize(wirePlan([wireGoal(1, "dispute_resolution", [
    wireField("disputeTopic", "quantity", "količini"),
    { i: engine._test.fieldIdByKey.disputeDescription, v: null, o: 60102, e: "količini" },
  ], "spor o količini")], "spor o količini"), {}, "rešita spor o količini");
  assert.equal(wrongDisputeDescriptionOption, null, "optionId 60102 ne sme preiti iz disputeTopic v disputeDescription");
  var nullValueWithEvidence = engine._test.materialize(wirePlan([wireGoal(1, "installment_plan", [
    { i: engine._test.fieldIdByKey.installmentAmount, v: null, o: null, e: "po 769" },
  ], "2 plačil po 769")], "2 plačil po 769"), { remainingDebt: 1538 }, "2 plačil po 769");
  assert.equal(nullValueWithEvidence, null, "evidence ne sme nadomestiti manjkajoče materialne vrednosti");
  var technicalScalarFormats = engine._test.materialize(wirePlan([wireGoal(1, "partial_payment_now", [
    { i: engine._test.fieldIdByKey.requestedAmount, v: "135 EUR", o: null, e: "135 evrov" },
    wireField("paymentDeadline", "2026-08-29", "danes"),
    wireField("remainingStrategy", "installments", "na obroke"),
    { i: engine._test.fieldIdByKey.remainingDeadline, v: null, o: null, e: "" },
  ], "danes 135 evrov na obroke")], "danes 135 evrov na obroke"), { remainingDebt: 635 }, "danes 135 evrov na obroke");
  assert.equal(technicalScalarFormats[0].goalData.requestedAmount, "135", "valutna pripona v tehničnem amount zapisu se sme odstraniti");
  assert.equal(technicalScalarFormats[0].goalData.remainingDeadline, undefined, "prazno opcijsko polje ostane prazno brez izmišljene evidence");
  var punctuatedReference = engine._test.materialize(wirePlan([wireGoal(1, "compensation", [
    wireField("compensationAmount", "145", "145 evrov"),
    { i: engine._test.fieldIdByKey.counterclaimReference, v: "P-84...", o: null, e: "P-84..." },
  ], "pobot 145 evrov P-84...")], "pobot 145 evrov P-84..."), {}, "pobot 145 evrov P-84...");
  assert.equal(punctuatedReference[0].goalData.counterclaimReference, "P-84", "končno stavčno tropičje ni del reference");
  ...
  var indexedSource = "lej pravno oceno poti";
  assert.deepEqual(engine._test.lexicalSourceTokens(indexedSource).map(function (token) { return [token.id, token.text]; }), [[1, "lej"], [2, "pravno"], [3, "oceno"], [4, "poti"]]);
  assert.equal(engine._test.resolveEvidence(indexedSource, "@2:4"), "pravno oceno poti", "Lunin token span se mora mehansko izrezati iz nespremenjenega vira");
  assert.equal(engine._test.resolveEvidence(indexedSource, "@4:2"), null, "obrnjen evidence span mora fail-closed");
  assert.equal(engine._test.resolveEvidence(indexedSource, "@2:9"), null, "evidence span zunaj vira mora fail-closed");
  assert.equal(engine._test.resolveEvidence(indexedSource, "pravna ocena poti"), null, "adapter ne sme popravljati sklona ali fuzzy iskati evidence");
  var indexedPlan = wirePlan([wireGoal(1, "legal_recovery", [wireField("legalOutcome", "legal_route_review", "@2:4")], "@2:4")], "@1:4");
  var indexedGoals = engine._test.materialize(indexedPlan, { remainingDebt: 434 }, indexedSource);
  assert.equal(indexedGoals.length, 1);
  assert.equal(indexedGoals[0].evidence, "pravno oceno poti");
  assert.equal(indexedGoals[0].goalData.legalOutcome, "legal_route_review");
  assert.equal(engine._test.installmentTargetAmount("653", "4"), "2612");
  assert.equal(engine._test.installmentTargetAmount("500", "4"), "2000");
  assert.equal(engine._test.installmentTargetAmount("500.001", "4"), null);
  assert.equal(engine._test.installmentTargetAmount("500", "1"), null);
  var derivedInstallmentSource = "4 obroki po 500";
  var derivedInstallment = engine._test.materialize(wirePlan([wireGoal(1, "installment_plan", [
    wireField("installmentAmount", "500", "po 500"), wireField("installmentCount", "4", "4 obroki"),
  ], derivedInstallmentSource)], derivedInstallmentSource), { remainingDebt: 2612 }, derivedInstallmentSource);
  assert.equal(derivedInstallment[0].goalData.targetAmount, "2000", "produkt obroka in števila ima prednost pred drugačnim kontekstnim dolgom");
  assert.ok(derivedInstallment[0].derivedFields.includes("targetAmount"));
  var contextInstallmentSource = "plačilo v štirih obrokih";
  var contextInstallment = engine._test.materialize(wirePlan([wireGoal(1, "installment_plan", [
    wireField("installmentCount", "4", "štirih obrokih"),
  ], contextInstallmentSource)], contextInstallmentSource), { remainingDebt: 2612 }, contextInstallmentSource);
  assert.equal(contextInstallment[0].goalData.targetAmount, "2612", "brez izračunljivega produkta se uporabi strukturiran kontekstni dolg");
  var explicitInstallmentSource = "cilj 1800 v 4 obrokih po 500";
  var explicitInstallment = engine._test.materialize(wirePlan([wireGoal(1, "installment_plan", [
    wireField("targetAmount", "1800", "cilj 1800"), wireField("installmentAmount", "500", "po 500"), wireField("installmentCount", "4", "4 obrokih"),
  ], explicitInstallmentSource)], explicitInstallmentSource), { remainingDebt: 2612 }, explicitInstallmentSource);
  assert.equal(explicitInstallment[0].goalData.targetAmount, "1800", "eksplicitnega ciljnega zneska se ne sme prepisati");
  assert.ok(!explicitInstallment[0].derivedFields.includes("targetAmount"));
  var indexedRequest = engine.requestBody(indexedSource, { remainingDebt: 434 }, "indexed-evidence");
  assert.deepEqual(JSON.parse(indexedRequest.input).sourceTokens, ["lej", "pravno", "oceno", "poti"]);
  assert.equal(JSON.parse(indexedRequest.input).sourceTokenCount, 4);
  assert.match(indexedRequest.instructions, /@firstTokenId:lastTokenId/);
  assert.match(indexedRequest.instructions, /MATERIAL VALUE AUDIT/);
  assert.match(indexedRequest.instructions, /FORWARD DEADLINE ATTACHMENT/);
  assert.match(indexedRequest.instructions, /TOP-LEVEL COMPLETENESS/);
  assert.equal(engine.RESPONSE_SCHEMA.properties.x.pattern, "^@[1-9][0-9]*:[1-9][0-9]*$");
  ...
  var repairCalls = 0;
  var repairedAfterInvalid = await engine.analyze("2 obroka po 769", { remainingDebt: 1538 }, { apiKey: "test", fetchImpl: async function () {
    repairCalls += 1;
    if (repairCalls === 1) return response({ p: [{ n: 1, c: 3, k: 1, f: [{ i: 301, v: null, o: null, e: "po 769" }], e: "2 obroka po 769" }], q: null, x: "2 obroka po 769" });
    return response(wirePlan([
      wireGoal(1, "installment_plan", [wireField("installmentAmount", "769", "po 769"), wireField("installmentCount", "2", "2 obroka")], "2 obroka po 769"),
      wireGoal(2, "installment_plan", [wireField("installmentAmount", "769", "po 769"), wireField("installmentCount", "2", "2 obroka")], "2 obroka po 769"),
    ], "2 obroka po 769"));
  } });
  assert.equal(repairCalls, 2, "neveljaven modelski contract dobi največ eno odzivno ponovitev");
  assert.equal(repairedAfterInvalid.goals.length, 2);
  assert.equal(repairedAfterInvalid.semanticPlan.transport.responseRetries, 1);
  var invalidCalls = 0;
  await assert.rejects(engine.analyze("2 obroka po 769", { remainingDebt: 1538 }, { apiKey: "test", fetchImpl: async function () { invalidCalls += 1; return response({}); } }), function (error) { return error.code === "LUNA_INVALID_GOAL_PLAN"; });
  assert.equal(invalidCalls, 2, "po eni neuspešni odzivni ponovitvi mora tok ostati fail-closed");
  ...
  assert.match(ui, /contractVersion !== "goal-fact-v18"/);
  ...
  assert.match(ui, /function izracunajEnakObrok/);
  assert.match(ui, /osveziSamodejniZnesekObroka/);
  assert.match(ui, /trenutni && trenutni !== ciljSamodejniZnesekObroka/);
