...
if (approvedCount !== 62) throw new Error(`Kanonični register NAZORJEVA mora imeti natanko 62 potrjenih widgetov, trenutno: ${approvedCount}.`);
if (totalCount !== approvedCount + draftCount) throw new Error("Skupno število zasnov se ne ujema z vsoto potrjenih in testnih.");
...
if (!widgets || widgets.category !== "3.0" || widgets.records.length !== totalCount) throw new Error("Kategorija 3.0 mora vsebovati vseh 62 Ateninih widgetov.");
const amount = (label, value = "2.400,00") => `<div class="uj-card-field"><span class="uj-card-label">${esc(label)}</span><label class="uj-card-money uj-card-money--unit-only"><input type="text" inputmode="decimal" value="${esc(value)}" aria-label="${esc(label)}"><b>€</b></label></div>`;
...
const area = (label, placeholder) => `<label class="uj-card-field uj-card-field--wide"><span class="uj-card-label">${esc(label)}</span><textarea rows="3" placeholder="${esc(placeholder)}"></textarea></label>`;
...
## Kategorija 3.0 — Atenini widgeti

Kategorija 3.0 vsebuje vseh **${widgets.records.length} kanoničnih interaktivnih widgetov** iz istega Ateninega rendererja.

| Oznaka | Status | ID | Zasnova | Primer vprašanja | Pokritje |
|---:|---|---|---|---|---|
${widgets.records.map((template) => `| 3.0.${String(template.number).padStart(2, "0")} | ${template.approved ? "Potrjeno" : "Nov osnutek"} | ${md(template.id)} | **${md(template.title)}** | ${md(template.question)} | ${md(template.coverage)} |`).join("\n")}
...
<body class="atena-template-page">
  ...
  <main>
    ...
    <header class="uj-template-header"><h2>Kategorija 3.0 — Atenini widgeti</h2><p>Vseh ${widgets.records.length} obstoječih interaktivnih widgetov iz istega kanoničnega Ateninega rendererja.</p></header>
    <section class="uj-template-gallery" data-template-gallery data-widget-gallery aria-label="${widgets.records.length} Ateninih widgetov kategorije 3.0">${templates.renderGallery("3.0")}</section>
  </main>
