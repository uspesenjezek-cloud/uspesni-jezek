...
function assertSafeCard(result, expected) {
  ...
  assert.equal(result.semanticPlan.transport.adapter, "atena-ai-sdk-v1");
  assert.equal(result.semanticPlan.transport.calls, 1);
  ...
  assert.equal(structuredContracts.verifyEvidenceSpan(structuredContracts.createDocument(result.candidates[0].evidence.sourceSpan.text), result.candidates[0].evidence.span), true);
...
async function main() {
  ...
  assert.match(request.instructions, /Never write a free-form clarification question/);
  assert.deepEqual(parser.CLARIFICATION_CODES, ["agreement_kind", "accepted_or_proposed", "promise_or_refusal"]);
  ...
  var clarification = await parser.analyze("dogovor ni dovolj jasen", context, { apiKey: "test", fetchImpl: lunaResponse([], "agreement_kind", "dogovor ni dovolj jasen") });
  assert.equal(clarification.needsClarification, true);
  assert.equal(clarification.clarification.code, "agreement_kind");
  assert.equal(clarification.clarification.question, "Kaj je dolžnik dejansko sprejel ali povedal?");
  ...
  assert.equal(parser._test.materializeLunaProposal({ agreements: [Object.assign({}, agreement("payment_promise", 100, null, "unknown"), { evidence: "x" })], question: "agreement_kind", evidence: "x" }, context, "x"), null, "plan in vprašanje se ne smeta pojaviti skupaj");
  var exhausted = parser._test.clarificationResult({ agreements: [], question: "agreement_kind", evidence: "x" }, { remainingDebt: 9446, clarification: { round: 2 } }, "x");
  ...
  assert.match(history, /Atena history's only semantic authority/);
  ...
  assert.match(source, /require\("\.\/atena-ai-sdk"\)/);
  assert.equal(parser.RESPONSE_ZOD_SCHEMA.safeParse({ agreements: [], question: null, evidence: null }).success, false);
  assert.equal(parser.RESPONSE_ZOD_SCHEMA.safeParse({ agreements: [], question: "agreement_kind", evidence: "dogovor" }).success, true);
  assert.equal(parser.RESPONSE_ZOD_SCHEMA.safeParse({ agreements: [], question: "Poljubno vprašanje?", evidence: "dogovor" }).success, false);
