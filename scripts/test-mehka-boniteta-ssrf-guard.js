...
(async function () {
  ...
  assert.doesNotMatch(handlerSource, /async function poisciImpressumZBrskalnikom/,
    "kritična identitetna pot ne sme več odpirati ločenega brskalniškega iskanja");
  assert.match(handlerSource, /async function zajemiDokaziloIdentitete[\s\S]*?pripraviBrskalniskoStran\(browser[\s\S]*?namestiVarovaloJavnihPuppeteerZahtev\(stran\)[\s\S]*?stran\.goto/,
