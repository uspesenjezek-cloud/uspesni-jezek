...
`partial_payment_now` podpira tudi sestavljen cilj »del do prvega roka, preostanek do drugega roka«. Polje `paymentDeadline` je rok prvega delnega plačila, `remainingStrategy=new_deadline` izbere nov rok za preostanek, `remainingDeadline` pa hrani ta drugi rok. Delež, kot je »polovico«, Luna izračuna iz trenutnega preostalega dolga; cilj ostane samo predlog in ne zmanjša knjiženega dolga. Če Luna za enum vrne veljaven eksplicitni `optionId`, adapter vedno shrani njegov kanonični ključ, tudi če je spremljevalna vrednost numerični ID ali lokalizirana oznaka. Denarni scalar sme imeti le tehnično pripono `EUR` oziroma `€`; reference ohranijo vsebino in odstranijo samo končno stavčno ločilo. Neobvezno prazno polje ne potrebuje izmišljenega evidence izseka.
